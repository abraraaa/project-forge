...
- **Home headline no longer renders the strength-day subtitle twice.** Both the italic accent line and the smaller descriptor below were rendering `viewSession.subtitle` ("Squat & Push" appeared as both the big line AND the small line). The small descriptor is now only rendered when it differs from the headline — strength days lose the duplicate; non-strength days keep the modality detail ("60 min at conversational pace…").
