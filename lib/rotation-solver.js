...
// is a preference.
function candidatesFor(key, slot, history, claimed, { relaxMemory = false } = {}) {
...
export function solveRotation({
  ...
  const opts = { history, focus, mainLifts, rng, temperature, repairPasses };
...
function solveOnce({ history, focus, mainLifts, rng, temperature, repairPasses, relaxMemory }) {
  const config = {};
  ...
  const slots = Object.entries(EXERCISE_POOLS)
    ...
    .map(([key, slot]) => ({ key, slot, n: candidatesFor(key, slot, history, claimed, { relaxMemory }).length }))
    .sort((a, b) => a.n - b.n);
  ...
  for (const { key, slot } of slots) {
    const candidates = candidatesFor(key, slot, history, claimed, { relaxMemory });
    const objectives = candidates.map((ex) => {
      const trial = { ...config, [key]: ex };
      return volumeObjective(volumeFor(trial, focus, mainLifts), focus);
    });
  ...
  for (let pass = 0; pass < repairPasses; pass++) {
    ...
    if (hardViolations(volume, focus).length === 0) break;
    ...
    const bestSwap = (relaxMemory) => {
      ...
      for (const [key, slot] of Object.entries(EXERCISE_POOLS)) {
        ...
        for (const ex of candidatesFor(key, slot, history, others, { relaxMemory })) {
          if (ex.name === config[key]?.name) continue;
          const trial = { ...config, [key]: ex };
          const cost = volumeObjective(volumeFor(trial, focus, mainLifts), focus);
  ...
  return {
    config,
    report: { volume, bands, objective: volumeObjective(volume, focus), outOfBand, memoryRelaxed: relaxMemory },
  };
}
