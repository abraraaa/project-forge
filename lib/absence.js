...
import { localDateStr, todayLocalIso } from "./dates.js";
...
// Parse "YYYY-MM-DD" at local noon — immune to DST edges. The noon anchor
// keeps addDays correct across DST, but the result must be FORMATTED locally
// (localDateStr), not via toISOString — at UTC+13/14 local noon is already the
// next UTC day, so the old toISOString().slice(0,10) shifted the returned date.
