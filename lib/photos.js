...
/** Pure: JPEG structure check — SOI marker (FF D8 FF) at the HEAD and EOI
 *  marker (FF D9) at the TAIL. The head-only check let "FF D8 FF + arbitrary
 *  payload" through (a zip/junk body wearing a JPEG hat); requiring the tail
 *  marker too rejects that class for a few bytes of work and no decode. The
 *  server never decodes the image, so this is the strongest validation that
 *  stays O(1) — deliberately NOT a full parse. The client re-encodes every
 *  input to JPEG, so JPEG-only here matches what we actually store; widening
 *  to other formats would only widen the surface. (2026-07-27) */
export function isJpegBytes(bytes) {
  if (!bytes || bytes.length < 4) return false;
  const soi = bytes[0] === 0xff && bytes[1] === 0xd8 && bytes[2] === 0xff;
...
export async function uploadPhoto(profile, authToken, date, blob, { bodyweightAt = null } = {}) {
  try {
    ...
    // Bodyweight rides a header, not the URL: query strings land in access logs.
    /** @type {Record<string, string>} */
    const headers = { "Content-Type": "image/jpeg", "X-HW-Auth": authToken || "" };
    if (bodyweightAt != null) headers["X-HW-Bodyweight"] = String(bodyweightAt);
...
export async function fetchPhotoIndex(profile, authToken) {
  try {
    ...
    // heldPhotos: an earlier holder of this name left photos behind. Passed
    // through as the bare fact the server reports — no count, no dates.
    return { ok: true, photos: body.photos || [], heldPhotos: !!body.heldPhotos };
  } catch {
    return { ok: false };
  }
}
...
export async function fetchPhotoObjectUrl(profile, authToken, date) {
  try {
    // The upload budget, not the API default: this is a full-resolution
    // binary, and 15s over a weak cell signal aborts mid-body — which
    // painted a permanent white frame (boss report, 2026-08-04).
