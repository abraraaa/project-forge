...
export async function uploadPhoto(profile, authToken, date, blob, { bodyweightAt = null } = {}) {
  try {
    ...
    const res = await fetch(`/api/photos?${params}`, {
      method: "POST",
      headers: { "Content-Type": "image/jpeg", "X-Forge-Auth": authToken || "" },
      body: blob,
    });
...
export async function fetchPhotoIndex(profile, authToken) {
  try {
    const res = await fetch(`/api/photos?profile=${encodeURIComponent(profile)}`, {
      headers: { "X-Forge-Auth": authToken || "" },
    });
...
export async function fetchPhotoObjectUrl(profile, authToken, date) {
  try {
    const res = await fetch(`/api/photos?profile=${encodeURIComponent(profile)}&date=${encodeURIComponent(date)}`, {
      headers: { "X-Forge-Auth": authToken || "" },
    });
