...
/**
 * @param {{ history?: any[], trainingState?: any, focus?: string, mainLifts?: Record<string,string>,
...
export function buildCoachContext({ history = [], trainingState = null, focus = "Forged", mainLifts = {}, week = [], bodyweightKg = null, now = new Date() } = {}) {
  ...
  const swaps = Object.entries(mainLifts || {}).filter(([k, v]) => v && v !== k).map(([k, v]) => `${v} (for ${k})`);
  out.push("", "## Setup");
  ...
  for (const rec of recent) {
    ...
    out.push(`- ${rec.date} · ${rec.readiness || "normal"}${rec.travel ? " · travelling" : ""}: ${exs.join("; ")}`);
  }
