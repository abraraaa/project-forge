...
import { SESSIONS, applyFocusToSessions, applyMainLiftsToSessions, DEFAULT_FOCUS } from "./programme.js";
...
// Landmarks live in ./volume-targets.js so lib/programme.js can read them
// without a cycle; re-exported here so every existing importer is unchanged.
import { VOLUME_TARGETS } from "./volume-targets.js";
export { VOLUME_TARGETS };

/**
 * What the landmarks rest on. Same shape as TEMPO_SOURCES so the citation
 * JSON-LD is built the same way on both content surfaces.
 *
 * Honest about provenance: the DOSE-RESPONSE is peer-reviewed, the specific
 * per-muscle numbers are practitioner-derived and are presented as such. A
 * fabricated DOI on a coaching heuristic would be worse than no citation.
 */
export const VOLUME_SOURCES = {
  schoenfeld2017: {
    cite: "Schoenfeld BJ, Ogborn D, Krieger JW. Dose-response relationship between weekly resistance training volume and increases in muscle mass: a systematic review and meta-analysis. J Sports Sci. 2017;35(11):1073-1082.",
    url: "https://doi.org/10.1080/02640414.2016.1210197",
    note: "Establishes the graded relationship between weekly sets and hypertrophy, with meaningful growth clustering from roughly ten sets per muscle per week.",
  },
  schoenfeld2016freq: {
    cite: "Schoenfeld BJ, Ogborn D, Krieger JW. Effects of resistance training frequency on measures of muscle hypertrophy: a systematic review and meta-analysis. Sports Med. 2016;46(11):1689-1697.",
    url: "https://doi.org/10.1007/s40279-016-0543-8",
    note: "Weekly volume is the dose, but it is better split than stacked: major muscle groups respond best trained at least twice a week. Whether three beats two is, on this evidence, undetermined.",
  },
  israetel: {
    cite: "Israetel M, Hoffmann J, Smith CW. Scientific Principles of Hypertrophy Training. Renaissance Periodization, 2021.",
    url: null,
    note: "The MEV/MAV/MRV vocabulary and the per-muscle starting landmarks. Practitioner-derived, not a controlled trial — treated here as a well-reasoned starting point, not a measurement.",
  },
};
 ...
 * @param {{focus?:string, config?:object, mainLifts?:object}} [opts]
 ...
 *   mainLifts — the user's chosen anchor movements. Applied BEFORE focus, in
 *            the same order the session pipeline uses, because the choice
 *            changes which muscles the anchor credits: Weighted Dips over
 *            Bench moves chest and triceps. Counting the template's mains for
 *            a user who picked otherwise audits a week nobody trains.
 * @returns {Record<string, number>}  muscle → weighted sets/week (1dp).
 */
export function computeWeeklyVolume(
  sessions = SESSIONS,
  { focus = DEFAULT_FOCUS, config = {}, mainLifts = {} } = {},
) {
  const chosen = applyMainLiftsToSessions(sessions, mainLifts);
  const adjusted = applyFocusToSessions(chosen, focus, config, mainLifts);
 ...
 * @param {Object} [opts.mainLifts]       Chosen anchor movements
...
export function auditVolume(
  sessions = SESSIONS,
  { targets = VOLUME_TARGETS, focus = DEFAULT_FOCUS, config = {}, mainLifts = {} } = {},
) {
  const volume = computeWeeklyVolume(sessions, { focus, config, mainLifts });
  const muscles = [
    ...AUDIT_MUSCLE_ORDER,
...
export function auditHistoryVolume(
  ...
  // ISO strings compare lexicographically, so the window is a plain string
  // compare — no Date parse of rec.date (which was `new Date("YYYY-MM-DD")`,
  // a UTC-midnight parse that excluded the boundary day west of Greenwich).
  const oldest = addDaysIso(now, -weeks * 7);

  const inWindow = (history || []).filter((rec) => rec?.date >= oldest);
