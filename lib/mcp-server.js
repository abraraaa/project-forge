...
const INSTRUCTIONS = "Heatwayve is a strength-training app. These tools read the user's own training, read only. Start with training_snapshot: it explains how the programme works before the numbers. Use programme for what they're running next. Progress photos are never available here.";
...
/** One exercise as a prescription line fragment. */
function exLine(ex, sets, weights, reps) {
  const r = reps[ex.name] ?? ex.reps;
  const w = weights[ex.name] ?? ex.weight;
  const load = w == null || w === 0 ? "bodyweight"
    : `${w} kg${ex.loadType === "per_db" ? " each" : ""}${weights[ex.name] == null ? " (suggested start)" : ""}`;
...
export function describeProgramme(meta, history, todayIso) {
  ...
  for (const template of SESSIONS) {
    ...
    for (const b of session.blocks || []) {
      const sets = b.sets || 0;
      if (b.ex) out.push(`- ${b.label}: ${exLine(b.ex, sets, weights, reps)}`);
      else if (b.exA && b.exB) out.push(`- ${b.label}: ${exLine(b.exA, sets, weights, reps)} + ${exLine(b.exB, sets, weights, reps)}`);
    }
  }
