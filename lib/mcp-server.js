...
export function weekOn(history, todayIso) {
  let best = null;
  // Same shapes the app accepts, including a legacy bare 7-day array.
  for (const e of ensureScheduleHistory(history) || []) {
    if (!(e.effectiveFrom <= todayIso)) continue;
...
export async function handleMcp(msg, { load, now = new Date() }) {
  ...
  switch (method) {
    ...
    case "tools/call": {
      ...
      try { r = runTool(name, params?.arguments || {}, await load(), now); }
