...
import { WEEK, SESSIONS, recordLetterIdx, nextStrengthIdx, sessionMetaForDate } from "./programme.js";
 ...
 * | { kind: "tick", type: DayType, plannedAtTick: DayType|null }} Did */
...
function didOn(ctx, iso) {
  ...
  if (entry?.completedType) return { kind: "tick", type: entry.completedType, plannedAtTick: entry.scheduledType || null };
  return null;
}

// A tick satisfies its own type. On a PAST day it also stands if it matched
// the plan in force when it was made (Days.scheduledType): a later week edit
// (effective from Monday) must not turn a zone2 you did into an owed strength
// day that can only be cleared by logging a session you never did. Today
// stays strict — tick cardio, flip today to strength, and Begin is reachable.
const satisfies = (did, plannedType, when = "today") =>
  did?.kind === "strength" ||
  (did?.kind === "tick" && (did.type === plannedType || (when === "past" && !!did.plannedAtTick && did.type === did.plannedAtTick)));
...
function resolveIsoWeek(ctx, mondayIso) {
  ...
  const missed = base.filter((d) => d.when === "past" && d.planned === "strength" && !satisfies(d.did, d.planned, d.when) && !d.resting);
  ...
  const out = base.map((d) => {
    const done = satisfies(d.did, d.planned, d.when);
 ...
 * Strength rows are named plain "Strength": the letter is chosen when the day is
 * logged (the retro sheet shows it), and per-row letters read as nonsense when
 * several missed days share the next one.
 * @param {DayContext} ctx
...
export function owedDays(ctx, { daysBack = 7 } = {}) {
  ...
  for (let i = 1; i <= daysBack; i++) {
    ...
    out.push({
      ...
      sessionName: s ? "Strength" : (st.plannedDay?.label || meta.sessionName || st.planned),
...
/**
 * The home rhythm (StreakLine) over [today-(n-1), today]: every non-rest
 * planned day, conditioning included, so "Done. Rhythm kept." on a ticked
 * cardio day is true. completed = done or made up (a covered strength day
 * counts: the surplus session both completes its own day and makes this one
 * up). expected = those days once past (today only when done), minus resting
 * days not trained anyway. Off-plan sessions on rest days count on neither
 * side unless they cover a miss. strengthRhythm stays strength-only for the
 * Lab and deload.
 * @param {DayContext} ctx
 * @param {{ days?: number }} [opts]
 * @returns {{ completed: number, expected: number, ratio: number, window: number }}
 */
export function trainingRhythm(ctx, { days = 28 } = {}) {
  const range = resolveRange(ctx, addDaysIso(ctx.todayIso, -(days - 1)), ctx.todayIso)
    .filter((d) => d.planned !== "rest");
  const completed = range.filter((d) => d.done || d.covered).length;
  const expected = range.filter((d) => d.done || d.covered || (d.when === "past" && !d.resting)).length;
  return { completed, expected, ratio: Math.min(1, completed / Math.max(1, expected)), window: days };
}
 ...
 * is the number of the week's days inside a breather; plannedResting the
 * strength days a breather took out of planned (drawn as rest, not missed).
 * @param {DayContext} ctx
 * @param {{ weeks?: number }} [opts]
 * @returns {{ mondayIso: string, planned: number, plannedSoFar: number, plannedResting: number,
 *   done: number, resting: number, partial: boolean }[]}
 */
export function weeklyStrength(ctx, { weeks = 8 } = {}) {
  ...
  for (let w = weeks - 1; w >= 0; w--) {
    ...
    out.push({
      ...
      plannedResting: days.filter((d) => d.planned === "strength" && d.resting && !d.done).length,
...
/**
 * Back-compat wrapper: one ISO week from the old argument shape. Letters are
 * the resolver's own (past missed days take cycle-before-date).
...
export function resolveWeek({ mondayIso, todayIdx, history = [], days = {}, weekFor, breaks = [] }) {
  ...
  return resolveRange(ctx, mondayIso, addDaysIso(mondayIso, 6));
}
