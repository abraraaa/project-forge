...
//     itself. Never hard-code a second grid.
...
function applyMovement(decision, currentWeight, category, profile) {
  ...
  switch (decision) {
    case "ADD":
      return Math.max(0, currentWeight + step);
    case "DROP_5":
      return Math.max(0, roundToCategoryIncrement(currentWeight * 0.95, category));
    case "DROP_10":
      return Math.max(0, roundToCategoryIncrement(currentWeight * 0.90, category));
    case "HOLD":
    default:
