 ...
 * @property {number} consecutiveLightMisses  One-set misses in a row; the third drops the lift
...
import { weeklyRhythm } from "./analytics.js";
...
// Consecutive one-set misses before the lift drops 5% instead of holding.
const LIGHT_MISS_ESCALATION = 3;
...
export function computeNextPrescription({
  ...
  if (isBodyweightMovement(loadType) && !(lastWeight > 0)) {
    ...
    return {
      // 0, not null: "no added load" is a value. A null left the stored
      // working weight alone, so a stale total (bodyweight) survived every
      // unloaded session and kept pre-filling as added load.
      weight: 0,
      reps: repsNext,
      sets: lastEx.prescribed?.sets ?? DEFAULT_SETS,
      rir: DEFAULT_RIR,
  ...
  // Loaded progression
  let movement = decideMovement(performance, rir, addThresholdRir, context.readiness ?? lastReadiness);
  // A single-set miss holds, and holds forever: the lift never drops, it just
  // racks up consecutiveHolds until the whole programme offers a deload for
  // one stubborn exercise. Three in a row on the same lift is a load problem
  // on that lift, and gets that lift's answer — 5% off, counters drained.
  if (
    movement.reason === "missed_light" &&
    (liftState.consecutiveLightMisses ?? 0) >= LIGHT_MISS_ESCALATION - 1
  ) {
    movement = { decision: "DROP_5", reason: "missed_light_repeated" };
  }
  ...
  let baseReps = prevRange?.baseReps ?? programmeReps;
  let heldReps = prevRange?.reps ?? programmeReps;
  // The user set their own target: the session carries the engine's target
  // (repsForSession), so a first set logged at a different count is the
  // user's choice. It becomes this lift's base — the engine climbs from it
  // and never snaps back to the programme's number.
  const userReps = targetRepsFor(lastEx, lastEx.sets);
  if (prevRange && userReps > 0 && userReps !== heldReps) {
    baseReps = userReps;
    heldReps = userReps;
    rationale.push("user_rep_target");
  }
...
// The movement reason rides in the rationale trail as `decision_reason=…`;
// that is the prescription's contract, so read it rather than widen the type.
function movementReasonOf(prescription) {
  const hit = (prescription?.rationale || []).find((r) => typeof r === "string" && r.startsWith("decision_reason="));
  return hit ? hit.slice("decision_reason=".length) : null;
}
...
export function updateLiftStateFromSession(liftState, sessionRecord, exercise, prescription) {
  ...
  // One-set misses in a row. Cooked freezes it for the same reason it freezes
  // consecutiveHolds; any other outcome — including the drop the third miss
  // earns — ends the run.
  let consecutiveLightMisses = prevState.consecutiveLightMisses ?? 0;
  const reason = movementReasonOf(prescription);
  if (reason === "missed_light") consecutiveLightMisses += 1;
  else if (reason !== "readiness_cooked") consecutiveLightMisses = 0;
...
export function reconcileLiftStateWithSession(liftState, exercise) {
  ...
  const sets = exercise?.sets || [];
  // Bodyweight movements store ADDED load everywhere a weight is kept or
  // prescribed (the unit the user enters). effectiveLoad includes bodyweight:
  // right for e1RM and volume, wrong here — reconciling from it is how a
  // plain pull-up's currentWeight became the lifter's bodyweight (2026-09-25).
  const loadType = exercise?.loadType ?? sets.find((s) => s?.loadType)?.loadType ?? null;
  if (isBodyweightMovement(loadType)) {
    if (sets.length === 0) return liftState;
    const added = Math.max(0, ...sets.map((s) => (Number.isFinite(s?.weight) ? s.weight : 0)));
    return added === liftState.currentWeight ? liftState : { ...liftState, currentWeight: added };
  }
  const loads = sets
...
// A deload answers accumulated fatigue, and fatigue needs training to
// accumulate. Every signal above counts sessions, never time — four HOLDs
// spread across three sporadic months read exactly like four in a month —
// so an irregular lifter's absence manufactures the offer, and deloading the
// undertrained compounds the detraining. Flat window on purpose: a new user
// cannot pass it for ~10 weeks at full adherence, which is where the first
// deload belongs anyway.
const DELOAD_REGULARITY_WEEKS         = 12;
const DELOAD_REGULARITY_MIN_PER_WEEK  = 2.5;                      // distinct training days, mean over the window
...
// ─── trainingRegularity ───────────────────────────────────────────────────────
// Mean distinct training days per week over the trailing window, using the
// same per-week count the adherence strip draws (weeklyRhythm), so "regular"
// here is the number the user already sees.
export function trainingRegularity(history, weeks = DELOAD_REGULARITY_WEEKS) {
  const cols = weeklyRhythm(history || [], weeks);
  const total = cols.reduce((n, c) => n + c.days, 0);
  const perWeek = cols.length ? total / cols.length : 0;
  return { perWeek, weeks: cols.length, regular: perWeek >= DELOAD_REGULARITY_MIN_PER_WEEK };
}
...
//   - If the trailing 12 weeks average under 2.5 training days → no offer
//     (signals still detect; the Performance Lab can still say "stalled")
export function shouldOfferDeload(trainingState, history) {
  ...
  if (!trainingRegularity(history).regular) return null;
