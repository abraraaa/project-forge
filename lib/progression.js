...
import { getLiftProfile, STEP_SIZES, ADD_THRESHOLD_RIR, coldStartFromAnchor, snapToImplement } from "./lift-translations.js";
...
function applyMovement(decision, currentWeight, category, profile, loadType = null) {
  ...
  switch (decision) {
    case "ADD":
      return Math.max(0, snap(currentWeight + step));
    case "DROP_5":
...
export function computeNextPrescription({
  ...
  const loadType = context.loadType
    ...
    ?? null;
  const nextWeight = applyMovement(movement.decision, lastWeight, category, profile, loadType);
