...
export async function dbInsertToken(token, { profile, expires, scope = null, createdAt, authAt = null }) {
  const q = sql();
  if (!q) return false;
  await ensureSchema(q);
  await q`INSERT INTO auth_tokens (token, profile, expires, scope, created_at, auth_at)
          VALUES (${token}, ${profile}, ${expires}, ${scope}, ${createdAt || new Date().toISOString()}, ${authAt})
...
export async function dbReadToken(token) {
  ...
  if (!q) return null;
  await ensureSchema(q);
  const rows = await q`SELECT profile, expires, scope, created_at, auth_at FROM auth_tokens WHERE token = ${token}`;
