...
/** One photo's index row, or null. The GET path reads the STORED blob_path
 *  from here rather than recomputing it from (profile, date): a recomputed
 *  path is guessable, so a profile with no index rows could still have its
 *  predecessor's photos fetched a date at a time. No row, no photo. */
...
// ─── Retiring a lapsed profile's photos ──────────────────────────────────────
// NOT A DELETE. This rewrites the profile key on a photo's index rows so the
// name's new holder cannot list or fetch them, while every row and every blob
// survives intact. Recovery is the same statement in reverse.
//
// It exists because progress photos are keyed by profile NAME, and a name is
// transferable: once a lapsed profile is re-claimed, the new holder would
// otherwise inherit the previous person's gallery. Refusing that is worth
// doing; destroying the photos to achieve it is not, because the returning
// owner has as strong a claim as the stranger has a weak one.
//
// The retired key contains "/", which no profile name may hold (rejected by
// isBadProfileName in the photos route and validateProfile in sync), so a
// retired row can never collide with — or be addressed as — a real profile.
...
/** Whether any earlier era's photos are held under this name. Prefix-compared
 *  with left(), NOT LIKE: a profile name may legally contain % or _, which
 *  LIKE would read as wildcards and match other people's rows. */
