...
/** Read a profile from the DB: { meta, history } | null if no rows at all. */
export async function dbReadProfile(profile) {
  ...
  return { meta: assembleMeta(metaRows), history: sessRows.map((r) => r.record) };
}

/** Upsert a full profile snapshot (merged upstream). Sessions are immutable:
 *  ON CONFLICT DO NOTHING. Meta fields overwrite (the caller already ran the
 *  stamp-aware merge). Never deletes. */
...
/** DESTRUCTIVE (wipe-protocol: announced 2026-07-19, rides the existing
 *  passkey-gated profile wipe only): delete a profile's DB rows. Exactly
 *  the scope the blob DELETE already covers; enumerated tables, no glob. */
