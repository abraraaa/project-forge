...
export function checkAuthorizeParams(q, client) {
  // The usual cause is a Client ID typed into the AI's advanced OAuth
  // fields: that skips self-registration, and we never issued that ID.
  if (!client) return { fatal: "This AI didn't register itself with Heatwayve — usually because a Client ID was typed into its connector settings. Remove the connector, add it again with only the name and URL, and leave the OAuth fields empty." };
