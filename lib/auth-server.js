...
export function rpConfigFromRequest(request) {
  ...
  const origin = ALLOWED_ORIGIN_HOSTS.has(host)
    ? `https://${host}`
    : "https://theforged.fit"; // unknown host: fail toward the legacy origin
  return { rpId: "theforged.fit", expectedOrigin: origin };
}
 ...
 * @param {{ profile: string, ttlMs: number, scope?: string | null }} opts
 */
export async function mintAuthToken({ profile, ttlMs, scope = null }) {
...
export async function readTokenData(authToken) {
  ...
  // DB-first (Rec 11b); blob fallback covers tokens minted before the
  // migration — the longest-lived (7-day photo cookies) age out naturally,
  // after which the fallback is a dead branch for the deep audit to prune.
  if (hasDb()) {
    const row = await dbReadToken(authToken);
    if (row) return row;
  }
