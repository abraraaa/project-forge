...
/**
 * Whether the profile holds a passkey that can still complete a ceremony:
 * verifiable AND bound to an rpId that is still servable.
 *
 * WHY THIS IS SEPARATE FROM hasRealPasskey, and stays separate.
 *
 * After the sunset a theforged.fit credential is unusable — the browser can no
 * longer fetch the ROR document at the rpId origin, so the ceremony cannot
 * complete. Nothing is deleted; it simply stops being a key. Surfaces that ask
 * "can this person get in?" must use THIS predicate, or they would demand a
 * passkey that can never work and lock the owner out of their own name.
 *
 * The WIPE GATE deliberately keeps using hasRealPasskey. A profile that was
 * ever protected must not become deletable by a stranger merely because its
 * lock rusted: re-registration is how the owner returns, and destruction is
 * not something a rusted lock should open. Loosening the claim path is the
 * boss's ruling; loosening the delete path is not, and never falls out of it
 * by accident.
 *
...
/**
 * Is this registration a RE-CLAIM of a profile whose lock has lapsed?
 *
 * True only when the profile already holds credential records but none of them
 * can still complete a ceremony — so whoever is registering could not have
 * proved control, and may not be the previous owner. That is the one case in
 * which the previous owner's progress photos must stop being reachable under
 * this name.
 *
 * FALSE for everything else, and the false cases are the ones that matter:
 *   · a first-ever claim (nothing to inherit)
 *   · adding a second passkey (proved control — same person)
 *   · the rpId UPGRADE flow, which also proves control first
 * Getting this wrong in the other direction would retire the photos of every
 * user who simply upgraded their passkey, which is why it is a pure function
 * with its own tests rather than a condition inlined in a route.
 *
