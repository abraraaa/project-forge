...
import { mondayOfWeekIso, addDaysIso } from "./dates.js";
...
export function weeklyVolume(history) {
  const byWeek = {};
  for (const rec of history || []) {
    const weekStart = mondayOfWeekIso(rec.date);
...
export function weeklyRhythm(history, weeks = 12) {
  // Local Monday of the current week, via the shared helper (this function's
  // own local math was already correct — it's the reference the audit named;
  // now it shares the one implementation instead of re-deriving it).
  const monday = mondayOfWeekIso(new Date());

  const cols = [];
  for (let w = weeks - 1; w >= 0; w--) {
    const start = addDaysIso(monday, -w * 7);
    cols.push({ weekStart: start, _end: addDaysIso(start, 6), _days: new Set() });
  }
  for (const rec of history || []) {
...
// ─── Session counts ───────────────────────────────────────────────────────────
// `travel` counts records the session host marked `travel: true` (lib/travel.js).
// They are ordinary sessions everywhere volume is concerned — the sets happened
// — but lib/progression.js refuses them as load evidence. Counting them here
// lets aggregate surfaces say so instead of leaving a flat e1RM unexplained.
export function sessionCount(history) {
  const today = Date.now();
  const sevenAgo  = today - 7  * 86400000;
  const thirtyAgo = today - 30 * 86400000;
  let total = 0, last7 = 0, last30 = 0, travel = 0;
  for (const rec of history || []) {
    total += 1;
    if (rec?.travel === true) travel += 1;
    const t = new Date(rec.id).getTime();
    if (t >= sevenAgo) last7 += 1;
    if (t >= thirtyAgo) last30 += 1;
  }
  return { total, last7, last30, travel };
}
  ...
  for (const rec of sorted) {
    ...
    for (const block of rec.blocks || []) {
      for (const ex of block.exercises || []) {
        ...
        const sets = (ex.sets || []).filter(isCountedSet);
...
export function weeklyVolumeByMuscle(history = [], { weeks = 8, now = new Date() } = {}) {
  ...
  // Local-day arithmetic throughout (lib/dates.js doctrine). The scaffold
  // used to parse todayMon with `new Date(str)` (UTC midnight) and format
  // with `.toISOString().slice(0,10)` (UTC) — so across a DST transition the
  // emitted column label slipped a day off its Monday and stopped matching
  // mondayOfWeekIso(rec.date), which is computed in LOCAL time. Records in
  // those weeks hit `weekIndex[undefined]` and were silently dropped from
  // the histogram. Reproduced: from 2026-11-02 in Europe/London, the old
  // scaffold emitted 2026-10-18 where records hash to 2026-10-19.
  const todayMon = mondayOfWeekIso(now);
  const weekStarts = [];
  for (let w = weeks - 1; w >= 0; w--) {
    weekStarts.push(addDaysIso(todayMon, -w * 7));
  }
  ...
  for (const rec of history || []) {
    ...
    const ws = mondayOfWeekIso(rec.date);
...
// ─── Weekly tonnage wave (Home) ──────────────────────────────────────────────
// Per-week kg moved across the last N calendar weeks (Mon-anchored, local,
// same bucketing as weeklyRhythm; same per-set maths as totalTonnage so the
// surfaces agree). Powers the Home tonnage sparkline — oldest week first,
// current (partial) week last. Weeks with no sessions are 0, not skipped.
export function weeklyTonnage(history = [], weeks = 6) {
  const monday = mondayOfWeekIso(new Date());
  const cols = [];
  for (let w = weeks - 1; w >= 0; w--) {
    const start = addDaysIso(monday, -w * 7);
    cols.push({ weekStart: start, _end: addDaysIso(start, 6), kg: 0 });
  }
  for (const rec of history || []) {
    if (!rec?.date) continue;
    const col = cols.find((c) => rec.date >= c.weekStart && rec.date <= c._end);
    if (!col) continue;
    for (const block of rec.blocks || []) {
      for (const ex of block.exercises || []) {
        for (const s of ex.sets || []) {
          const mult = loadTypeMultiplier(s.loadType ?? ex.loadType);
          if (s.volume != null) {
            col.kg += s.volume * mult;
          } else {
            const reps = parseReps(s.reps);
            const load = s.effectiveLoad ?? s.weight;
            if (load && reps) col.kg += load * reps * mult;
          }
        }
      }
    }
  }
  return cols.map(({ weekStart, kg }) => ({ weekStart, kg: Math.round(kg) }));
}
...
// Re-cut 2026-08-04 (boss): the Home tonnage sparkline now carries the
// week-to-week story, so the card keeps only the RARE beats — a milestone
// is a ceremony, not wallpaper. Anyone already past a threshold when this
// shipped sees at most one card (highest newly-crossed), same seen logic.
export const TONNAGE_MILESTONES_KG = [
  100_000,     // 100 tonnes
  500_000,
  1_000_000,   // a thousand tonnes
  2_000_000,
  5_000_000,
  10_000_000,
];
...
// Date helpers live in lib/dates.js now. `mondayOfWeek` here previously did
// `new Date("YYYY-MM-DD")` (UTC-parse) then `.toISOString()` (UTC-format),
// which bucketed a Monday session into the prior week and produced Tuesday
// week keys for every user west of UTC — the exact class the rhythm-grid fix
// named but this copy kept. `mondayOfWeekIso` is the local-correct one.
...
export function computeVolumeAggregates(history, { anchor = Date.now() } = {}) {
  ...
  // Count how many of the four trailing 28-day sub-windows ACTUALLY contain
  // records, and normalise by that. The prior form was `floor(112/28)` = a
  // constant 4 whenever any record existed, so a user with 5 weeks of history
  // had their baseline divided by 4 — reading ~4× too low and classifying a
  // normal week as "elevated". Dividing by real coverage restores a true
  // 28-day-equivalent. (Still dormant — no consumer yet — but no longer wrong
  // for the moment it's wired up.)
  let baselineWindowsCovered = 0;
  for (let i = 0; i < 4; i++) {
    const winAnchor = baselineEndAnchor - i * 28 * MS_PER_DAY;
    if (recordsInWindow(history, 28, winAnchor).length > 0) baselineWindowsCovered++;
  }
  baselineWindowsCovered = Math.max(1, baselineWindowsCovered);
