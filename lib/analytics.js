...
import { localDateStr, parseLocalDate, mondayOfWeekIso } from "./dates.js";
...
export function weeklyRhythm(history, weeks = 12) {
  ...
  const monday = parseLocalDate(mondayOfWeekIso(new Date()));
  ...
  for (let w = weeks - 1; w >= 0; w--) {
    ...
    cols.push({ weekStart: localDateStr(start), _end: localDateStr(end), _days: new Set() });
  }
  for (const rec of history || []) {
...
export function weeklyVolumeByMuscle(history = [], { weeks = 8, now = new Date() } = {}) {
  ...
  for (let w = weeks - 1; w >= 0; w--) {
    ...
    weekStarts.push(localDateStr(d));
  }
...
export function weeklyTonnage(history = [], weeks = 6) {
  ...
  for (let w = weeks - 1; w >= 0; w--) {
    ...
    cols.push({ weekStart: localDateStr(start), _end: localDateStr(end), kg: 0 });
  }
  for (const rec of history || []) {
    if (!rec?.date) continue;
