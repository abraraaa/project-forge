...
// for the squat day; bss2 (Bulgarian + Ham Curl) duplicates ass2's hip work;
// css3 (DB Curl + Skullcrusher) is pure-arm isolation for the power day.
// All three drops preserve the main lifts + at least one accessory superset
// + the finisher, so the day still covers every major movement pattern.

const STRONG_DROP_BLOCK_IDS = new Set(["ass2", "bss2", "css3"]);
...
export function applySwapsToSession(session, swaps = {}) {
  ...
  const blocks = session.blocks.map((b) => {
    ...
    if (b.ex) {
      const s = swaps[b.id];
      if (s?.name) next = { ...next, ex: { ...b.ex, ...s } };
    }
    if (b.exA) {
      const s = swaps[`${b.id}-A`];
      if (s?.name) next = { ...next, exA: { ...b.exA, ...s } };
    }
    if (b.exB) {
      const s = swaps[`${b.id}-B`];
      if (s?.name) next = { ...next, exB: { ...b.exB, ...s } };
    }
    return next;
  });
  return { ...session, blocks };
  ...
  if (focus === "Strong") {
    const blocks = session.blocks
      ...
      .map(b => {
        ...
        const next = { ...b };
        if (next.exA) next.exA = { ...next.exA, reps: STRONG_ACCESSORY_REPS };
        if (next.exB) next.exB = { ...next.exB, reps: STRONG_ACCESSORY_REPS };
        if (next.ex)  next.ex  = { ...next.ex,  reps: STRONG_ACCESSORY_REPS };
        return next;
      });
    return { ...session, blocks };
  }
...
export const SWAP_DB = {
  ...
  // ───────────────────────────────────────────────────────────────────────────
  "45-Degree Hip Extension": [{ name:"Glute Bridge"                    , eq:"Bodyweight" , muscle:"Posterior chain"           , vid:null },
                               { name:"Romanian Deadlift"               , eq:"Barbell"    , muscle:"Posterior chain"           , vid:null }],
  "Assisted Pull-Up": [{ name:"Lat Pulldown"                    , eq:"Machine"    , muscle:"Lats"                      , vid:null },
                        { name:"Pull-Up"                         , eq:"Bodyweight" , muscle:"Lats"                      , vid:null }],
  "B-Stance Hip Thrust": [{ name:"Single-Leg Hip Thrust"           , eq:"Bodyweight" , muscle:"Glutes"                    , vid:null },
  ...
  "Band Face Pull": [{ name:"Resistance Band Face Pull"       , eq:"Band"       , muscle:"Rear delts"                , vid:null },
                      { name:"Rear Delt Fly"                   , eq:"Dumbbell"   , muscle:"Rear delts"                , vid:null }],
  "Band Lateral Raise": [{ name:"Resistance Band Lateral"         , eq:"Band"       , muscle:"Lateral delt"              , vid:null },
                          { name:"Lateral Raise"                   , eq:"Dumbbell"   , muscle:"Lateral delt"              , vid:null }],
  "Band Pull-Apart": [{ name:"Resistance Band Face Pull"       , eq:"Band"       , muscle:"Rear delts"                , vid:null },
                       { name:"Rear Delt Fly"                   , eq:"Dumbbell"   , muscle:"Rear delts"                , vid:null }],
  "Banded Hip Thrust": [{ name:"Glute Bridge"                    , eq:"Bodyweight" , muscle:"Glutes"                    , vid:null },
                         { name:"Barbell Hip Thrust"              , eq:"Barbell"    , muscle:"Glutes"                    , vid:null }],
  "Barbell Front Rack Lunge": [{ name:"DB Reverse Lunge"                , eq:"Dumbbell"   , muscle:"Quads & Glutes"            , vid:null },
                                { name:"Step-Up"                         , eq:"Bodyweight" , muscle:"Quads & Glutes"            , vid:null }],
  "Barbell Glute Bridge": [{ name:"Glute Bridge"                    , eq:"Bodyweight" , muscle:"Glutes"                    , vid:null },
                            { name:"Barbell Hip Thrust"              , eq:"Barbell"    , muscle:"Glutes"                    , vid:null }],
  "Barbell Step-Up": [{ name:"Step-Up"                         , eq:"Bodyweight" , muscle:"Quads & Glutes"            , vid:null },
                       { name:"DB Step-Up"                      , eq:"Dumbbell"   , muscle:"Quads & Glutes"            , vid:null }],
  "Barbell Walking Lunge": [{ name:"DB Walking Lunge"                , eq:"Dumbbell"   , muscle:"Quads & Glutes"            , vid:null },
                             { name:"Reverse Lunge"                   , eq:"Bodyweight" , muscle:"Quads & Glutes"            , vid:null }],
  "Belt Squat": [{ name:"Goblet Squat"                    , eq:"Dumbbell"   , muscle:"Quads & Glutes"            , vid:null },
                  { name:"Leg Press"                       , eq:"Machine"    , muscle:"Quads & Glutes"            , vid:null }],
  "Bench Dips": [{ name:"Tricep Pushdown"                 , eq:"Cable"      , muscle:"Triceps"                   , vid:null },
                  { name:"Close-Grip Push-Up"              , eq:"Bodyweight" , muscle:"Triceps"                   , vid:null }],
  "Cable Crossover": [{ name:"DB Chest Fly"                    , eq:"Dumbbell"   , muscle:"Chest / medial"            , vid:null },
                       { name:"Pec Deck"                        , eq:"Machine"    , muscle:"Chest / medial"            , vid:null }],
  "Cable Overhead Extension": [{ name:"DB Overhead Extension"           , eq:"Dumbbell"   , muscle:"Triceps"                   , vid:null },
                                { name:"Skullcrusher"                    , eq:"Barbell"    , muscle:"Triceps"                   , vid:null }],
  "Cable Rear Delt Fly": [{ name:"Rear Delt Fly"                   , eq:"Dumbbell"   , muscle:"Rear delts"                , vid:null },
                           { name:"Resistance Band Face Pull"       , eq:"Band"       , muscle:"Rear delts"                , vid:null }],
  "Cable Straight-Arm Pulldown": [{ name:"Lat Pulldown"                    , eq:"Machine"    , muscle:"Lats"                      , vid:null },
                                   { name:"Resistance Band Pull-Down"       , eq:"Band"       , muscle:"Lats"                      , vid:null }],
  "Captain's Chair Raise": [{ name:"Hanging Knee Raise"              , eq:"Bodyweight" , muscle:"Core"                      , vid:null },
                             { name:"Lying Leg Raise"                 , eq:"Bodyweight" , muscle:"Core"                      , vid:null }],
  "Chin-Up": [{ name:"Pull-Up"                         , eq:"Bodyweight" , muscle:"Lats"                      , vid:null },
               { name:"Lat Pulldown"                    , eq:"Machine"    , muscle:"Lats"                      , vid:null }],
  "Close-Grip Push-Up": [{ name:"Diamond Push-Up"                 , eq:"Bodyweight" , muscle:"Triceps"                   , vid:null },
                          { name:"Bench Dips"                      , eq:"Bodyweight" , muscle:"Triceps"                   , vid:null }],
  "Concentration Curl": [{ name:"DB Curl"                         , eq:"Dumbbell"   , muscle:"Biceps"                    , vid:null },
                          { name:"Hammer Curl"                     , eq:"Dumbbell"   , muscle:"Biceps"                    , vid:null }],
  "Curtsy Lunge": [{ name:"DB Reverse Lunge"                , eq:"Dumbbell"   , muscle:"Quads & Glutes"            , vid:null },
                    { name:"Reverse Lunge"                   , eq:"Bodyweight" , muscle:"Quads & Glutes"            , vid:null }],
  "DB Chest Fly": [{ name:"Cable Crossover"                 , eq:"Cable"      , muscle:"Chest / medial"            , vid:null },
                    { name:"Pec Deck"                        , eq:"Machine"    , muscle:"Chest / medial"            , vid:null }],
  "DB Floor Press": [{ name:"Dumbbell Bench Press"            , eq:"Dumbbell"   , muscle:"Chest"                     , vid:null },
                      { name:"Close-Grip Push-Up"              , eq:"Bodyweight" , muscle:"Chest"                     , vid:null }],
  "DB Front Squat": [{ name:"Goblet Squat"                    , eq:"Dumbbell"   , muscle:"Quads & Glutes"            , vid:null },
                      { name:"Barbell Back Squat"              , eq:"Barbell"    , muscle:"Quads & Glutes"            , vid:null }],
  "DB Lateral Lunge": [{ name:"DB Reverse Lunge"                , eq:"Dumbbell"   , muscle:"Quads & Glutes"            , vid:null },
                        { name:"Reverse Lunge"                   , eq:"Bodyweight" , muscle:"Quads & Glutes"            , vid:null }],
  "DB Lu Raise": [{ name:"Lateral Raise"                   , eq:"Dumbbell"   , muscle:"Lateral delt"              , vid:null },
                   { name:"Resistance Band Lateral"         , eq:"Band"       , muscle:"Lateral delt"              , vid:null }],
  "DB Overhead Extension": [{ name:"Cable Overhead Extension"        , eq:"Cable"      , muscle:"Triceps"                   , vid:null },
                             { name:"Skullcrusher"                    , eq:"Barbell"    , muscle:"Triceps"                   , vid:null }],
  ...
  "DB Step-Up": [{ name:"Step-Up"                         , eq:"Bodyweight" , muscle:"Quads & Glutes"            , vid:null },
                  { name:"DB Reverse Lunge"                , eq:"Dumbbell"   , muscle:"Quads & Glutes"            , vid:null }],
  "DB Sumo Squat": [{ name:"Goblet Squat"                    , eq:"Dumbbell"   , muscle:"Quads & Glutes"            , vid:null },
                     { name:"Barbell Back Squat"              , eq:"Barbell"    , muscle:"Quads & Glutes"            , vid:null }],
  "Decline Push-Up": [{ name:"Incline DB Press"                , eq:"Dumbbell"   , muscle:"Upper chest"               , vid:null },
                       { name:"Pike Push-Up"                    , eq:"Bodyweight" , muscle:"Upper chest"               , vid:null }],
  "Deficit Reverse Lunge": [{ name:"DB Reverse Lunge"                , eq:"Dumbbell"   , muscle:"Quads & Glutes"            , vid:null },
                             { name:"Reverse Lunge"                   , eq:"Bodyweight" , muscle:"Quads & Glutes"            , vid:null }],
  "Diamond Push-Up": [{ name:"Close-Grip Push-Up"              , eq:"Bodyweight" , muscle:"Triceps"                   , vid:null },
                       { name:"Bench Dips"                      , eq:"Bodyweight" , muscle:"Triceps"                   , vid:null }],
  "Donkey Calf Raise": [{ name:"Leg Press Calf Raise"            , eq:"Machine"    , muscle:"Calves"                    , vid:null },
                         { name:"Standing Calf Raise"             , eq:"Machine"    , muscle:"Calves"                    , vid:null }],
  "Dumbbell Bent-Over Row": [{ name:"Chest-Supported DB Row"          , eq:"Dumbbell"   , muscle:"Upper back"                , vid:null },
                              { name:"TRX Row"                         , eq:"Equipment"  , muscle:"Upper back"                , vid:null }],
  "EZ Bar Curl": [{ name:"DB Curl"                         , eq:"Dumbbell"   , muscle:"Biceps"                    , vid:null },
                   { name:"Hammer Curl"                     , eq:"Dumbbell"   , muscle:"Biceps"                    , vid:null }],
  "Face-Away Cable Row": [{ name:"Seated Cable Row"                , eq:"Cable"      , muscle:"Mid back"                  , vid:null },
                           { name:"Dumbbell Bent-Over Row"          , eq:"Dumbbell"   , muscle:"Mid back"                  , vid:null }],
  "Frog Pump": [{ name:"Glute Bridge"                    , eq:"Bodyweight" , muscle:"Glutes"                    , vid:null },
                 { name:"Banded Hip Thrust"               , eq:"Band"       , muscle:"Glutes"                    , vid:null }],
  "Goblet Squat": [{ name:"DB Front Squat"                  , eq:"Dumbbell"   , muscle:"Quads & Glutes"            , vid:null },
                    { name:"Wall Sit"                        , eq:"Bodyweight" , muscle:"Quads & Glutes"            , vid:null }],
  "Hack Squat": [{ name:"Leg Press"                       , eq:"Machine"    , muscle:"Quadriceps"                , vid:null },
                  { name:"Goblet Squat"                    , eq:"Dumbbell"   , muscle:"Quadriceps"                , vid:null }],
  "Half-Kneeling Cable Row": [{ name:"Seated Cable Row"                , eq:"Cable"      , muscle:"Mid back"                  , vid:null },
                               { name:"Resistance Band Row"             , eq:"Band"       , muscle:"Mid back"                  , vid:null }],
  "Hammer Curl": [{ name:"DB Curl"                         , eq:"Dumbbell"   , muscle:"Biceps & brachialis"       , vid:null },
                   { name:"Zottman Curl"                    , eq:"Dumbbell"   , muscle:"Biceps & brachialis"       , vid:null }],
  "Hanging Knee Raise": [{ name:"Lying Leg Raise"                 , eq:"Bodyweight" , muscle:"Core"                      , vid:null },
                          { name:"Reverse Crunch"                  , eq:"Bodyweight" , muscle:"Core"                      , vid:null }],
  "Incline DB Curl": [{ name:"DB Curl"                         , eq:"Dumbbell"   , muscle:"Biceps"                    , vid:null },
                       { name:"EZ Bar Curl"                     , eq:"Barbell"    , muscle:"Biceps"                    , vid:null }],
  "Incline DB Row": [{ name:"Chest-Supported DB Row"          , eq:"Dumbbell"   , muscle:"Upper back"                , vid:null },
                      { name:"Dumbbell Bent-Over Row"          , eq:"Dumbbell"   , muscle:"Upper back"                , vid:null }],
  "Incline Landmine Press": [{ name:"Landmine Press"                  , eq:"Barbell"    , muscle:"Upper chest"               , vid:null },
                              { name:"Incline DB Press"                , eq:"Dumbbell"   , muscle:"Upper chest"               , vid:null }],
  "Kneeling Landmine Press": [{ name:"Landmine Press"                  , eq:"Barbell"    , muscle:"Upper chest"               , vid:null },
  ...
  "L-Sit Hold": [{ name:"Plank"                           , eq:"Bodyweight" , muscle:"Core"                      , vid:null },
                  { name:"Hanging Knee Raise"              , eq:"Bodyweight" , muscle:"Core"                      , vid:null }],
  "Landmine Squeeze Press": [{ name:"DB Chest Fly"                    , eq:"Dumbbell"   , muscle:"Upper chest"               , vid:null },
                              { name:"Cable Crossover"                 , eq:"Cable"      , muscle:"Upper chest"               , vid:null }],
  "Lat Pulldown": [{ name:"Pull-Up"                         , eq:"Bodyweight" , muscle:"Lats"                      , vid:null },
                    { name:"Resistance Band Pull-Down"       , eq:"Band"       , muscle:"Lats"                      , vid:null }],
  "Leaning Lateral Raise": [{ name:"Lateral Raise"                   , eq:"Dumbbell"   , muscle:"Lateral delt"              , vid:null },
                             { name:"Cable Lateral Raise"             , eq:"Cable"      , muscle:"Lateral delt"              , vid:null }],
  "Leg Extension": [{ name:"Goblet Squat"                    , eq:"Dumbbell"   , muscle:"Quadriceps"                , vid:null },
                     { name:"Wall Sit"                        , eq:"Bodyweight" , muscle:"Quadriceps"                , vid:null }],
  "Leg Press Calf Raise": [{ name:"Standing Calf Raise"             , eq:"Machine"    , muscle:"Calves"                    , vid:null },
                            { name:"Seated Calf Raise"               , eq:"Machine"    , muscle:"Calves"                    , vid:null }],
  "Low-to-High Cable Fly": [{ name:"Incline DB Press"                , eq:"Dumbbell"   , muscle:"Upper chest"               , vid:null },
                             { name:"Incline Push-Up"                 , eq:"Bodyweight" , muscle:"Upper chest"               , vid:null }],
  "Meadows Row": [{ name:"Dumbbell Bent-Over Row"          , eq:"Dumbbell"   , muscle:"Upper back"                , vid:null },
                   { name:"Single-Arm DB Row"               , eq:"Dumbbell"   , muscle:"Upper back"                , vid:null }],
  "Neutral-Grip DB Press": [{ name:"Dumbbell Bench Press"            , eq:"Dumbbell"   , muscle:"Chest"                     , vid:null },
                             { name:"DB Floor Press"                  , eq:"Dumbbell"   , muscle:"Chest"                     , vid:null }],
  "Neutral-Grip Pull-Up": [{ name:"Pull-Up"                         , eq:"Bodyweight" , muscle:"Lats"                      , vid:null },
                            { name:"Chin-Up"                         , eq:"Bodyweight" , muscle:"Lats"                      , vid:null }],
  "Nordic Curl": [{ name:"Dumbbell Leg Curl"               , eq:"Dumbbell"   , muscle:"Hamstrings"                , vid:null },
                   { name:"Swiss Ball Leg Curl"             , eq:"Bodyweight" , muscle:"Hamstrings"                , vid:null }],
  "Overhead Tricep Extension": [{ name:"DB Overhead Extension"           , eq:"Dumbbell"   , muscle:"Triceps"                   , vid:null },
                                 { name:"Tricep Pushdown"                 , eq:"Cable"      , muscle:"Triceps"                   , vid:null }],
  "Pec Deck": [{ name:"Cable Crossover"                 , eq:"Cable"      , muscle:"Chest / medial"            , vid:null },
                { name:"DB Chest Fly"                    , eq:"Dumbbell"   , muscle:"Chest / medial"            , vid:null }],
  "Pendulum Squat": [{ name:"Hack Squat"                      , eq:"Machine"    , muscle:"Quadriceps"                , vid:null },
                      { name:"Goblet Squat"                    , eq:"Dumbbell"   , muscle:"Quadriceps"                , vid:null }],
  "Preacher Curl": [{ name:"DB Curl"                         , eq:"Dumbbell"   , muscle:"Biceps"                    , vid:null },
                     { name:"Concentration Curl"              , eq:"Dumbbell"   , muscle:"Biceps"                    , vid:null }],
  "Prone Y Raise": [{ name:"Rear Delt Fly"                   , eq:"Dumbbell"   , muscle:"Rear delts"                , vid:null },
                     { name:"Resistance Band Face Pull"       , eq:"Band"       , muscle:"Rear delts"                , vid:null }],
  "Rear Delt Fly": [{ name:"Resistance Band Face Pull"       , eq:"Band"       , muscle:"Rear delts"                , vid:null },
                     { name:"Cable Rear Delt Fly"             , eq:"Cable"      , muscle:"Rear delts"                , vid:null }],
  "Reverse Hyperextension": [{ name:"Glute Bridge"                    , eq:"Bodyweight" , muscle:"Glutes"                    , vid:null },
                              { name:"Romanian Deadlift"               , eq:"Barbell"    , muscle:"Posterior chain"           , vid:null }],
  "Reverse-Grip Pushdown": [{ name:"Tricep Pushdown"                 , eq:"Cable"      , muscle:"Triceps"                   , vid:null },
                             { name:"Rope Tricep Pushdown"            , eq:"Cable"      , muscle:"Triceps"                   , vid:null }],
  "Rope Tricep Pushdown": [{ name:"Tricep Pushdown"                 , eq:"Cable"      , muscle:"Triceps"                   , vid:null },
                            { name:"Single-Arm Pushdown"             , eq:"Cable"      , muscle:"Triceps"                   , vid:null }],
  "Seal Row": [{ name:"Dumbbell Bent-Over Row"          , eq:"Dumbbell"   , muscle:"Upper back"                , vid:null },
                { name:"TRX Row"                         , eq:"Equipment"  , muscle:"Upper back"                , vid:null }],
  "Seated Calf Raise": [{ name:"Standing Calf Raise"             , eq:"Machine"    , muscle:"Calves"                    , vid:null },
                         { name:"Single-Leg Calf Raise"           , eq:"Bodyweight" , muscle:"Calves"                    , vid:null }],
  "Seated Lateral Raise": [{ name:"Lateral Raise"                   , eq:"Dumbbell"   , muscle:"Lateral delt"              , vid:null },
                            { name:"Cable Lateral Raise"             , eq:"Cable"      , muscle:"Lateral delt"              , vid:null }],
  "Seated Leg Curl": [{ name:"Machine Hamstring Curl"          , eq:"Machine"    , muscle:"Hamstrings"                , vid:null },
                       { name:"Dumbbell Leg Curl"               , eq:"Dumbbell"   , muscle:"Hamstrings"                , vid:null }],
  "Single-Arm Cable Fly": [{ name:"DB Chest Fly"                    , eq:"Dumbbell"   , muscle:"Chest / medial"            , vid:null },
                            { name:"Pec Deck"                        , eq:"Machine"    , muscle:"Chest / medial"            , vid:null }],
  "Single-Arm Cable Row": [{ name:"Seated Cable Row"                , eq:"Cable"      , muscle:"Mid back"                  , vid:null },
                            { name:"Single-Arm DB Row"               , eq:"Dumbbell"   , muscle:"Mid back"                  , vid:null }],
  "Single-Arm DB Row": [{ name:"Dumbbell Bent-Over Row"          , eq:"Dumbbell"   , muscle:"Upper back"                , vid:null },
                         { name:"TRX Row"                         , eq:"Equipment"  , muscle:"Upper back"                , vid:null }],
  "Single-Arm Landmine Press": [{ name:"Landmine Press"                  , eq:"Barbell"    , muscle:"Upper chest"               , vid:null },
                                 { name:"Dumbbell Shoulder Press"         , eq:"Dumbbell"   , muscle:"Upper chest"               , vid:null }],
  "Single-Arm Pushdown": [{ name:"Tricep Pushdown"                 , eq:"Cable"      , muscle:"Triceps"                   , vid:null },
                           { name:"Rope Tricep Pushdown"            , eq:"Cable"      , muscle:"Triceps"                   , vid:null }],
  "Single-Leg Calf Raise": [{ name:"Standing Calf Raise"             , eq:"Machine"    , muscle:"Calves"                    , vid:null },
                             { name:"Seated Calf Raise"               , eq:"Machine"    , muscle:"Calves"                    , vid:null }],
  "Single-Leg Curl": [{ name:"Dumbbell Leg Curl"               , eq:"Dumbbell"   , muscle:"Hamstrings"                , vid:null },
                       { name:"Slider Leg Curl"                 , eq:"Bodyweight" , muscle:"Hamstrings"                , vid:null }],
  "Single-Leg Hip Thrust": [{ name:"Glute Bridge"                    , eq:"Bodyweight" , muscle:"Glutes"                    , vid:null },
                             { name:"Barbell Hip Thrust"              , eq:"Barbell"    , muscle:"Glutes"                    , vid:null }],
  "Single-Leg RDL": [{ name:"Romanian Deadlift"               , eq:"Barbell"    , muscle:"Glutes / Hams"             , vid:null },
                      { name:"Glute Bridge"                    , eq:"Bodyweight" , muscle:"Glutes / Hams"             , vid:null }],
  "Sissy Squat": [{ name:"Goblet Squat"                    , eq:"Dumbbell"   , muscle:"Quadriceps"                , vid:null },
                   { name:"Wall Sit"                        , eq:"Bodyweight" , muscle:"Quadriceps"                , vid:null }],
  "Skullcrusher": [{ name:"DB Overhead Extension"           , eq:"Dumbbell"   , muscle:"Triceps"                   , vid:null },
                    { name:"Tricep Pushdown"                 , eq:"Cable"      , muscle:"Triceps"                   , vid:null }],
  "Slider Leg Curl": [{ name:"Swiss Ball Leg Curl"             , eq:"Bodyweight" , muscle:"Hamstrings"                , vid:null },
                       { name:"Dumbbell Leg Curl"               , eq:"Dumbbell"   , muscle:"Hamstrings"                , vid:null }],
  "Smith Calf Raise": [{ name:"Standing Calf Raise"             , eq:"Machine"    , muscle:"Calves"                    , vid:null },
                        { name:"Leg Press Calf Raise"            , eq:"Machine"    , muscle:"Calves"                    , vid:null }],
  "Svend Press": [{ name:"DB Chest Fly"                    , eq:"Dumbbell"   , muscle:"Chest / medial"            , vid:null },
                   { name:"Cable Crossover"                 , eq:"Cable"      , muscle:"Chest / medial"            , vid:null }],
  "Swiss Ball Leg Curl": [{ name:"Slider Leg Curl"                 , eq:"Bodyweight" , muscle:"Hamstrings"                , vid:null },
                           { name:"Dumbbell Leg Curl"               , eq:"Dumbbell"   , muscle:"Hamstrings"                , vid:null }],
  "Toes-to-Bar": [{ name:"Hanging Leg Raise"               , eq:"Bodyweight" , muscle:"Core"                      , vid:null },
                   { name:"Lying Leg Raise"                 , eq:"Bodyweight" , muscle:"Core"                      , vid:null }],
  "Tricep Pushdown": [{ name:"Rope Tricep Pushdown"            , eq:"Cable"      , muscle:"Triceps"                   , vid:null },
                       { name:"Close-Grip Push-Up"              , eq:"Bodyweight" , muscle:"Triceps"                   , vid:null }],
  "TRX Row": [{ name:"Dumbbell Bent-Over Row"          , eq:"Dumbbell"   , muscle:"Upper back"                , vid:null },
               { name:"Resistance Band Row"             , eq:"Band"       , muscle:"Upper back"                , vid:null }],
  "V-Squat": [{ name:"Leg Press"                       , eq:"Machine"    , muscle:"Quads & Glutes"            , vid:null },
               { name:"Goblet Squat"                    , eq:"Dumbbell"   , muscle:"Quads & Glutes"            , vid:null }],
  "Weighted Pull-Up": [{ name:"Pull-Up"                         , eq:"Bodyweight" , muscle:"Lats"                      , vid:null },
                        { name:"Lat Pulldown"                    , eq:"Machine"    , muscle:"Lats"                      , vid:null }],
  "Wide-Grip Cable Row": [{ name:"Seated Cable Row"                , eq:"Cable"      , muscle:"Mid back"                  , vid:null },
                           { name:"Dumbbell Bent-Over Row"          , eq:"Dumbbell"   , muscle:"Mid back"                  , vid:null }],
  "Wide-Grip Pull-Up": [{ name:"Pull-Up"                         , eq:"Bodyweight" , muscle:"Lats"                      , vid:null },
                         { name:"Lat Pulldown"                    , eq:"Machine"    , muscle:"Lats"                      , vid:null }],
  "Windshield Wiper": [{ name:"Hanging Knee Raise"              , eq:"Bodyweight" , muscle:"Core"                      , vid:null },
                        { name:"Plank"                           , eq:"Bodyweight" , muscle:"Core"                      , vid:null }],
  "Zottman Curl": [{ name:"DB Curl"                         , eq:"Dumbbell"   , muscle:"Biceps & forearms"         , vid:null },
                    { name:"Hammer Curl"                     , eq:"Dumbbell"   , muscle:"Biceps & forearms"         , vid:null }],
};
...
export function findUntickedRecent(history = [], daysBack = 7, dayDone = {}, { week = WEEK } = {}) {
  ...
  const scheduledStrengthCount = (week || WEEK).filter((d) => d?.type === "strength").length;
  const actualStrengthByWeek = {};
  for (const rec of history) {
    if (!rec?.date || !String(rec.session || "").startsWith("strength")) continue;
    const ws = localMondayOfWeek(rec.date);
  ...
  for (const r of rows) {
    ...
    if (r.type === "strength") {
      ...
      if ((actualStrengthByWeek[ws] || 0) >= scheduledStrengthCount) continue;
