...
import { VOLUME_TARGETS } from "./volume-targets.js";
...
export function recordLetterIdx(rec) {
...
export const SESSIONS = [
  // ── A · Monday · Squat + Push ─────────────────────────────────────────────
  {
    ...
    blocks:[
      ...
      // No accessory leg slot defaults to a spine-loaded barbell movement: the
      // main lift is where the tank gets emptied, and an accessory that makes
      // you hold back for it is the wrong accessory. The supine hip thrust in
      // ass2 is exempt — bar on the hips, nothing on the spine — and is the
      // programme's glute anchor. Held by tests/programme.test.js.
  ...
  // ── B · Wednesday · Hinge + Pull ──────────────────────────────────────────
  {
    ...
    blocks:[
      ...
      // 3, not 2: side-delt volume is almost all direct — this block and
      // css1-B are the slot — and at 2 sets the week sits on the MEV floor.
      { id:"bfin",type:"finisher",  label:"Finisher",            sets:3, rest:60,
...
// Which aligned superset blocks actually get Sculpt's +1 set this week.
//
// The bump used to fire on every aligned block unconditionally, and on the
// default config that put Glutes at 16.6 against an MRV of 16 — over the
// ceiling by construction, before rotation had picked anything. Measured
// across chained rotations it was the cause of Sculpt's out-of-band weeks
// (12% of solves) and of the solver spending variety to claw them back.
//
// So the bump is checked against the week it lands in: apply every candidate
// bump, tally the week, and while any muscle a kept bump feeds sits over its
// MRV, withdraw the bump from the kept block that feeds that muscle most.
// Largest-first means the fewest withdrawals; deterministic given block
// order. A block that loses its set keeps its 12–15 rep range — alignment is
// about reps, the ceiling is about sets. Returns the block ids that keep the
// set. Pure; same inputs, same set, on every surface that applies focus.
export function sculptBumpBlockIds(config = {}, mainLifts = {}) {
  const aligned = alignedSlotKeysForFocus(config, "Sculpt");
  const kept = new Set();
  if (aligned.size === 0) return kept;

  const week = SESSIONS.map((s) => applyMainLiftsToSession(applyRotationToSession(s, config), mainLifts));
  /** @type {Record<string, number>} */
  const totals = {};
  /** @type {Array<{ id: string, perSet: Record<string, number> }>} */
  const candidates = [];
  for (const s of week) {
    for (const b of s.blocks || []) {
      ...
      for (const key of ["ex", "exA", "exB"]) {
        const ex = b[key];
        if (!ex) continue;
        for (const [m, v] of Object.entries(distributeAcrossMuscles(ex.name, 1, ex.muscle))) {
          perSet[m] = (perSet[m] || 0) + v;
        }
      }
      for (const [m, v] of Object.entries(perSet)) totals[m] = (totals[m] || 0) + v * sets;
      const accessory = b.type !== "main" && b.type !== "finisher";
      if (accessory && (aligned.has(`${b.id}-A`) || aligned.has(`${b.id}-B`))) {
        candidates.push({ id: b.id, perSet });
        kept.add(b.id);
        for (const [m, v] of Object.entries(perSet)) totals[m] += v; // the +1
      }
    }
  }

  for (;;) {
    // Every landmarked muscle, not just the aligned ones: the bump lands on the
    // whole superset, so an aligned side can carry its partner's muscle over.
    const over = Object.keys(VOLUME_TARGETS)
      .filter((m) => (totals[m] || 0) > VOLUME_TARGETS[m].mrv + 1e-9)
      .sort((a, b) => (totals[b] - VOLUME_TARGETS[b].mrv) - (totals[a] - VOLUME_TARGETS[a].mrv));
    let best = null;
    for (const m of over) {
      for (const c of candidates) {
        if (!kept.has(c.id)) continue;
        const v = c.perSet[m] || 0;
        if (v > 0 && (!best || v > best.v)) best = { c, v };
      }
      // No kept bump feeds this muscle: the overage is baseline, not ours to
      // withdraw — leave it for the audit and try the next one.
      if (best) break;
    }
    if (!best) break;
    kept.delete(best.c.id);
    for (const [mm, v] of Object.entries(best.c.perSet)) totals[mm] -= v;
  }
  return kept;
}
...
// What each anchor slot trains, for the editor's section headings — the slot
// is a muscle group, the lift is a choice within it.
export const MAIN_LIFT_GROUPS = {
...
// One line for the profile row: which anchors differ from the programme.
// Push Press can fill two slots, so names are listed once and the count says
// how many slots changed.
export function mainLiftSummary(choices = {}) {
  const canon = Object.keys(MAIN_LIFT_FUNCTIONAL_EQUIVALENTS);
  const changed = canon.filter((c) => choices?.[c] && choices[c] !== c).map((c) => choices[c]);
  if (changed.length === 0) return "Programme defaults";
  return `${[...new Set(changed)].join(", ")} · ${changed.length} of ${canon.length} changed`;
}
...
export function isValidMainLiftChoice(canonical, choice) {
  ...
  if (!Object.hasOwn(MAIN_LIFT_FUNCTIONAL_EQUIVALENTS, canonical)) return false;
...
// @param {object} [config] programmeBlock.config (per-slot active exercise)
// @param {object} [mainLifts] chosen anchors (P.getMainLifts) — Sculpt's set
//                          ceiling is checked against the week they produce
// @returns {object}        adjusted session (same shape) — possibly with fewer
//                          blocks (Strong) and/or different sets/reps on blocks
export function applyFocusToSession(session, focus = DEFAULT_FOCUS, config = {}, mainLifts = {}, _bumped = null) {
  if (!session?.blocks) return session;
  ...
  if (focus === "Sculpt") {
    ...
    // applyFocusToSessions computes the week's kept set once and hands it
    // down; a single-session caller computes it here from the same inputs.
    const bumped = _bumped || sculptBumpBlockIds(config, mainLifts);
    const blocks = session.blocks.map(b => {
      if (b.type === "main" || b.type === "finisher") return b;
      ...
      // +1 set on the whole superset — unless the week's ceiling withdrew it.
      const next = { ...b, sets: bumped.has(b.id) ? b.sets + 1 : b.sets };
...
// the static template.
export function applyFocusToSessions(sessions = SESSIONS, focus = DEFAULT_FOCUS, config = {}, mainLifts = {}) {
  const bumped = focus === "Sculpt" ? sculptBumpBlockIds(config, mainLifts) : null;
  return sessions.map(s => applyFocusToSession(s, focus, config, mainLifts, bumped));
}
...
/**
 * No production callers: the app's picker reads owedDays (lib/day-state.js).
 * Kept one release as the parity reference for tests/day-resolver.test.js.
 *
...
export function findUntickedRecent(history = [], daysBack = 7, dayDone = {}, { week = WEEK, weekFor = null } = {}) {
  ...
  // Counted only up to YESTERDAY: what was due so far, not the whole week.
  // Against the full week, a session trained a day late (Tue for Mon) still
  // left Monday owed on Wednesday, because Wednesday and Friday were counted
  // as due before they had happened.
  const todayStr = todayLocalIso();
  const capByWeek = {};
  const scheduledStrengthCountFor = (weekStart) => {
    if (capByWeek[weekStart] === undefined) {
      ...
      for (let i = 0; i < 7; i++) {
        ...
        const w = resolveWeek(iso) || week || WEEK;
  ...
  // A session today belongs to today when today is itself a strength day;
  // only an off-schedule session today can stand in for an earlier miss.
  const todayWs = mondayOfWeekIso(todayStr);
  const todayIsStrength = (resolveWeek(todayStr) || week || WEEK)[mondayIndex(todayStr)]?.type === "strength";
  const trainedToday = history.some((r) => r?.date === todayStr && String(r.session || "").startsWith("strength"));
  if (todayIsStrength && trainedToday) actualStrengthByWeek[todayWs] = Math.max(0, (actualStrengthByWeek[todayWs] || 0) - 1);
