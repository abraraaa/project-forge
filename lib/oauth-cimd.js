...
/** A URL that failed is not refetched for this long (per instance). */
export const CIMD_FAIL_TTL_MS = 10 * 60 * 1000;
/** @type {Map<string, number>} */
const recentFailures = new Map();
/** Tests only: forget remembered failures. */
export const _resetCimdFailures = () => recentFailures.clear();
...
export async function resolveClient(store, clientId, { fetchImpl = fetch, now = Date.now() } = {}) {
  ...
  // A bad URL asked for again and again must not become repeat outbound fetches.
  const failedAt = recentFailures.get(clientId);
  if (failedAt && now - failedAt < CIMD_FAIL_TTL_MS) return known || null;
  let client = null;
  try { client = clientFromDocument(clientId, await fetchDocument(clientId, fetchImpl)); } catch { client = null; }
  if (!client) {
    recentFailures.set(clientId, now);
    if (recentFailures.size > 1000) recentFailures.delete(recentFailures.keys().next().value);
    return known || null;
  }
  recentFailures.delete(clientId);
