...
export function cacheAuthToken(profile, token, { admin = false } = {}) {
  ...
  // Device-local admin hint (boss find, 2026-07-28: the wing vanished in
  // standalone because the memory flag dies with the tab — the PWA rarely
  // re-runs a ceremony). The HINT may persist: it only reveals UI; every
  // admin API re-verifies the token's profile server-side, and a stale
  // hint buys an impostor a set of 403s. Wiped with the profile (suffix
  // registered in PROFILE_SUFFIXES).
  if (admin) {
    try { localStorage.setItem(`forge:${norm(profile)}:adminHint`, "1"); } catch {}
  }
}

/** UI hint only — every admin surface re-verifies server-side. True when
 *  this session proved admin, OR this device+profile has proven it before
 *  (persisted hint — visibility only; the tools still run ceremonies). */
export function isAdminSession(profile) {
  const hit = _cache.get(norm(profile));
  if (hit && Date.now() <= hit.expiresAt && hit.admin) return true;
  try { return localStorage.getItem(`forge:${norm(profile)}:adminHint`) === "1"; } catch { return false; }
}
