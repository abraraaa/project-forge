...
export function cacheAuthToken(profile, token) {
  if (!profile || !token) return;
  _cache.set(norm(profile), { token, expiresAt: Date.now() + TTL_MS });
}
...
export async function getAuthTokenWithCeremony(profile) {
  ...
  try {
    ...
    if (auth?.verified && auth?.authToken) {
      cacheAuthToken(profile, auth.authToken);
 ...
 * have — a cached token or nothing (the 30-day httpOnly cookie rides the
