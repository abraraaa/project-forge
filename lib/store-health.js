...
export const DEVICE_KEYS = new Set([
  ...
  // Device-level UI flags (written by ForgeApp) — were missing here, so
  // every user who crossed a tonnage milestone or dismissed the iOS install
  // banner got a standing false "unrecognised key" row on /diag-sync.
  "forge:tonnageMilestoneSeen", "forge:iosInstallDismissed",
  // Legacy appearance key from the first (device-level) cut of the theme
  // toggle — superseded by the per-profile `theme` suffix below before any
  // release. Recognised so a stray write never flags; never read.
  "forge:theme",
]);
export const PROFILE_SUFFIXES = new Set([
  ...
  "focusStamp", "trainingState", "breaks", "bodyweight", "bodyweightLog", "passkeyNudge",
  "draft", "pendingSession", "pendingRotationSummary",
  // Delta-sync bookkeeping: wiping a profile MUST clear
  // these — a recreated profile with a stale cursor would delta-pull from a
  // bogus position instead of hydrating fully.
  "syncCursor", "syncPushState", "adminHint",
  // Appearance override (lib/theme.js) — per-profile so a shared device
  // keeps each person's mode. Wiping a profile clears it with the rest.
  "theme",
  // Travel mode (lib/travel.js) — sticky because a trip lasts days, not one
  // session. Device-local on purpose: it describes where you are right now,
  // not who you are, so it must NOT ride the sync payload to the phone you
  // left at home.
  "travel",
  // Passkey RE-ENROLMENT prompt (lib/passkey-upgrade.js). Distinct from
  // "passkeyNudge" above and easily confused with it: that one asks a profile
  // with NO passkey to add one and lives on home; this one tells a profile
  // that HAS one, bound to the retiring rpId, that it needs replacing, and
  // lives on the profile page. Different audience, different trigger.
  "passkeyUpgrade",
]);
// Week-keyed legacy stores, abandoned in place by cutovers (never migrated,
// so old keys linger): bonusDone (cardio bonus marks) and weekDone (pre-Days
// completion projection, dropped in e754177). Recognised, never touched.
// Exported: the profile wipe iterates these too, so abandoned week-keyed
// entries don't survive a wipe as ghosts.
export const LEGACY_PROFILE_KEY_PREFIXES = ["bonusDone:", "weekDone:"];
const LEGACY_PREFIXES = LEGACY_PROFILE_KEY_PREFIXES;
...
export function checkStoreHealth(snapshot, { todayIso } = {}) {
  ...
  for (const [date, e] of Object.entries(s.days || {})) {
    ...
    // Time-invariant (audit 2026-07-26): the phantom persists forever, so the
    // detector must too — the old `&& date > today` clause stopped flagging it
    // the day after its date. `completedType && !sessionId && date > written`
    // is the exact, standing signature; it never matches a real completion or
    // a retro (whose written date is on/after the trained date).
    if (e.completedType && !e.sessionId && ISO_DATE.test(written) && date > written) {
  ...
  // localeCompare — the SAME order mergeHistories uses (audit 2026-07-26).
  // Bare .sort() is UTF-16 code-unit order, so a history correctly sorted by
  // the merge could be flagged 'not sorted' here.
  const sorted = [...ids].sort((a, b) => a.localeCompare(b)).every((id, i) => id === ids[i]);
...
export function collectStoreSnapshot(profile) {
  ...
  const unknownKeys = [];
  const knownSuffix = (suffix) =>
    PROFILE_SUFFIXES.has(suffix) || LEGACY_PREFIXES.some((p) => suffix.startsWith(p));
  const activePrefix = profile ? `forge:${profile}:` : null;
  for (let i = 0; i < window.localStorage.length; i++) {
    const k = window.localStorage.key(i);
    if (!k || !k.startsWith("forge:")) continue;
    if (DEVICE_KEYS.has(k)) continue;
    // The ACTIVE profile: strip its exact prefix and check the suffix. Robust
    // even if the profile name contains a ':' — the greedy `[^:]+` split below
    // mis-parsed those and flagged every one of that profile's keys as
    // unrecognised (audit P3, 2026-07-26).
    if (activePrefix && k.startsWith(activePrefix) && knownSuffix(k.slice(activePrefix.length))) continue;
    // Other-profile keys (multi-profile devices): best-effort greedy match.
    const m = k.match(/^forge:([^:]+):(.+)$/);
    if (m && knownSuffix(m[2])) continue;
    unknownKeys.push(k);
  }
  return {
