...
// implementation now lives in lib/sync-merge.js — THE merge, shared with
// the server PUT route (sync audit S3).
...
//        · pushNow (in the mutation handler) fires the sync
...
//
// SAVE POINTS that guarantee a SYNCED store reaches the cloud:
//   · per mutation              — pushNow inside the handler (delta-era:
//                                 dirty-diff decides what ships; free when clean).
//   · visibilitychange=hidden   — flushOnLifecycle (keepalive'd delta).
//   · pagehide                  — same.
// (The old class-1/class-2 deferral taxonomy is retired — #3, PR C;
// docs/push-refactor.md is historical.)
//
