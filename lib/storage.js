...
// Exported for tests — production callers go through mergeProfileData.
export function mergeDayEntries(localDays, remoteDays) {
  ...
  for (const date of dates) {
    ...
    // Both sides touched the same date — FIELD-AWARE merge, not whole-entry
    // latest-wins. A bonus mark on the phone and a completion on the iPad
    // are two facts about one day; whole-entry replacement made whichever
    // device synced last erase the other's fact. Latest-updatedAt entry
    // wins per-field conflicts; the loser's facts survive where the winner
    // has none. Safe because nothing un-completes or un-marks a day today —
    // there is no removal to preserve, only additions to union. Tie goes to
    // remote (cross-device canonical, matches the other meta fields).
