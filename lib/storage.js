...
export const Days = {
  ...
  projectCurrentWeek: (profile, { now = new Date() } = {}) => {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      const entry = all[iso];
      if (!entry) continue;
      if (entry.completedType) out.complete[i] = true;
  ...
  _maybeRepair: (profile) => {
    ...
    for (const [date, entry] of Object.entries(all)) {
      ...
      // Bonus-only entries (only `marks` populated, no completion fields) are
      // a legitimate state — user marked bonus but didn't tap Mark ✓ on the
      // scheduled training. Repairing those would mis-claim the scheduled
      // session as done. Leave them alone; if the user genuinely did both,
      // the retro picker will keep surfacing the day and one more Mark ✓ tap
      // resolves it correctly under the post-d6772c1 write path.
      if (entry.marks && Object.keys(entry.marks).length > 0) continue;
      const eff = W.getEffectiveOn(date) || DEFAULT_WEEK;
      const dow = _dowOfDate(date);
