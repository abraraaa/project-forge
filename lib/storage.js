 ...
 * @param {{weeklyStrengthDays?: number}} [opts]  Strength days in the user's
 *   current effective week. Callers derive it from W.get() || WEEK.
 */
export function computeRhythm(history, { weeklyStrengthDays = RHYTHM_DEFAULT_WEEKLY } = {}) {
  const now = Date.now();
  ...
  const expected = Math.max(1, weeklyStrengthDays || RHYTHM_DEFAULT_WEEKLY) * 4;
  return {
    completed,
    expected,
