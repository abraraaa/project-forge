...
import { todayLocalIso, mondayOfWeekIso, parseLocalDate, localDateStr, captureZone } from "./dates.js";
...
// Add N days to an ISO date string, returning ISO format. Composed from
// lib/dates.js — the hand-rolled parse+format here was locally correct but
// was a fourth copy of the same six lines, which is how this class keeps
// coming back (see tests/date-doctrine.test.js).
...
export function dateOfWeekdayIdxInCurrentWeek(weekdayIdx) {
  ...
  const d = parseLocalDate(mondayOfWeekIso(new Date()));
