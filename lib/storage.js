...
export function inferLoadType(name) {
  ...
  if (/(plank|hold|dead\s*bug|bird\s*dog|hollow|push-?up|crunch|raise|row\s*\(trx|trx|nordic|copenhagen|glute\s*bridge|side\s*plank|lateral\s*band|band\s*face\s*pull|(swiss\s*ball|stability\s*ball|slider)\s*leg\s*curl)/.test(n)) {
