...
import {
  mergeProfileData, mergeDayEntries, mergeHistories, fieldClosure,
  ensureScheduleHistory as _ensureScheduleHistory,
  scheduleEntryOn as _scheduleEntryOn,
  isValidWeekConfig, normaliseWeek,
} from "./sync-merge.js";
// Re-exported so existing consumers keep their import path; the implementation
// lives in lib/sync-merge.js — THE merge, shared with the server write path.
export { mergeDayEntries, mergeProfileData } from "./sync-merge.js";
import { activeBreak, breakEndedBy, makeBreak } from "./breaks.js";
import { todayLocalIso, mondayOfWeekIso, parseLocalDate, localDateStr, addDaysIso, mondayIndex, jsDow, captureZone } from "./dates.js";
import { fetchWithTimeout } from "./net.js";
import { isKnownPasskeyless } from "./passkeyless.js";
import { isCountedSet } from "./counted-set.js";
import { DeltaSync, needsReconcile } from "./sync-delta.js";
//
// ─── DURABILITY CONTRACT ────────────────────────────────────────────────────
// LOAD-BEARING. Read before adding a new store.
//
// Local storage is a write-through cache; the server copy is canonical.
// Nothing persists here without an explicit decision about how it survives a
// reinstall. Every store picks one of two dispositions:
//
//   1. SYNCED — must appear in ALL FOUR seams to round-trip safely:
//        · getLocalProfile()    reads the field into the snapshot
//        · persistToLocal()     writes it back from merged remote state
//        · mergeProfileData()   declares a merge rule
//        · the mutation handler fires a push
//      Missing any ONE of the four lets a reinstall silently revert the store
//      to its default, with no error anywhere.
//
//   2. DEVICE-LOCAL — intentionally not synced. Must carry an inline comment
//      saying so AND why it is safe to lose on reinstall.
//
// Adding a store without picking a side IS a regression. The live inventory
// and the enforcement both live in tests/storage.test.js#durability-contract —
// read it there rather than from a table here that can drift.
// ─────────────────────────────────────────────────────────────────────────────
...
export const LS = {
  ...
  set: (key, val) => {
    ...
    try {
      ...
      // Audit #10: this catch used to be EMPTY — a QuotaExceeded here meant
      // a finished session could silently vanish. Now: loud log, a sticky
      // flag /diag-sync and future UI can read, and a best-effort immediate
      // cloud push of the profile (the data exists in memory NOW; blob/DB
      // may still take it even though localStorage will not).
      console.error("[forge:LS.set] write FAILED (quota?)", key, e?.name || e);
      try {
        LS._quotaFailure = { key, at: new Date().toISOString(), name: e?.name || String(e) };
        // Monotonic counter, separate from the sticky record above: callers
        // that write a BATCH need to know whether any write in their own
        // window failed, which a last-failure record can't answer.
        LS._quotaFailures = (LS._quotaFailures || 0) + 1;
        const active = JSON.parse(localStorage.getItem("forge:active") || "null");
        if (active && !LS._quotaFlushing) {
          LS._quotaFlushing = true;
          Promise.resolve().then(() => import("./storage.js"))
            .then((m) => m.pushNow(active))
            .catch(() => {})
            .finally(() => { LS._quotaFlushing = false; });
        }
      } catch { /* the rescue itself must never throw */ }
      return false;
    }
  },
  remove: (key) => {
...
export const P = {
  ...
  // Chosen main lifts: canonical name -> the equivalent the user prefers.
  // Validated in lib/programme.js, not here — this is only the store.
  // SYNCED, stamped per lift (same algebra as weights): a phone changing
  // Squat and a laptop changing Bench offline both keep their edit.
  // Back-to-default is stored as canonical -> canonical, never a deleted
  // key: the merge is a union, so a deletion would be undone by the other
  // device's older copy.
  getMainLifts: (n)      => LS.get(`forge:${n}:mainLifts`, {}),
  getMainLiftStamps: (n) => LS.get(`forge:${n}:mainLiftStamps`, {}),
  setMainLift: (n, canonical, choice) => {
    const current = P.getMainLifts(n);
    // Re-tapping the selected chip changes nothing — and must not mint a
    // newer stamp that would beat another device's real edit.
    if ((current[canonical] || canonical) === (choice || canonical) && canonical in current) return current;
    const lifts = { ...current, [canonical]: choice || canonical };
    const stamps = { ...P.getMainLiftStamps(n), [canonical]: new Date().toISOString() };
    LS.set(`forge:${n}:mainLifts`, lifts);
    LS.set(`forge:${n}:mainLiftStamps`, stamps);
    return lifts;
  },
  saveMainLiftsRaw: (n, m, stamps) => {
    LS.set(`forge:${n}:mainLifts`, m || {});
    LS.set(`forge:${n}:mainLiftStamps`, stamps || {});
  },
  getWeights:   (n)      => LS.get(`forge:${n}:weights`, {}),
  // Weights/reps carry PER-KEY stamps (audit S2): without them, a pull
  // merged {...local, ...remote} and a stale blob regressed weights a
  // device had just earned offline. saveWeights/saveReps diff against the
  // stored map and stamp changed keys; the sync path writes merged maps +
  // merged stamps via the *Raw setters so a pull never re-stamps remote
  // values as freshly-local.
  saveWeights:  (n, w)   => {
    const prev = P.getWeights(n);
    const stamps = P.getWeightStamps(n);
    const now = new Date().toISOString();
    for (const [k, v] of Object.entries(w || {})) {
      if (prev[k] !== v) stamps[k] = now;
    }
    LS.set(`forge:${n}:weights`, w);
    LS.set(`forge:${n}:weightStamps`, stamps);
  },
  getWeightStamps: (n)   => LS.get(`forge:${n}:weightStamps`, {}),
  saveWeightsRaw: (n, w, stamps) => {
    LS.set(`forge:${n}:weights`, w || {});
    LS.set(`forge:${n}:weightStamps`, stamps || {});
  },
  getReps:      (n)      => LS.get(`forge:${n}:reps`, {}),
  saveReps:     (n, r)   => {
    const prev = P.getReps(n);
    const stamps = P.getRepStamps(n);
    const now = new Date().toISOString();
    for (const [k, v] of Object.entries(r || {})) {
      if (prev[k] !== v) stamps[k] = now;
    }
    LS.set(`forge:${n}:reps`, r);
    LS.set(`forge:${n}:repStamps`, stamps);
  },
  getRepStamps: (n)      => LS.get(`forge:${n}:repStamps`, {}),
  saveRepsRaw:  (n, r, stamps) => {
    LS.set(`forge:${n}:reps`, r || {});
    LS.set(`forge:${n}:repStamps`, stamps || {});
  },
...
// adherence ratio. Expected sessions derive from the USER'S schedule —
// weekly strength days × 4 weeks — not a hardcoded 3×/week: a 2-day plan
// capped at 67% adherence forever under the old constant, and a 4-day plan
// read 100% while under-training its own week. Missing a single day or even
// a whole week doesn't "break" anything — the number drops gracefully and
// recovers as you train again.
//
// Distinct DAYS count, not records — two sessions logged on one date are
// one day of adherence, not two.
//
// Returns: { completed: int, expected: int, ratio: float, window: 28 }
const RHYTHM_WINDOW_DAYS = 28;
const RHYTHM_DEFAULT_WEEKLY = 3; // matches the default WEEK's strength days

/**
 * @param {Array<Object>} history
 * @param {{weeklyStrengthDays?: number, weekFor?: (iso: string) => any}} [opts]
 *   weekFor resolves the schedule in force on a date; when given, expected is
 *   the strength days actually scheduled across the window, so a schedule
 *   edit doesn't re-judge past weeks by today's quota. weeklyStrengthDays is
 *   the fallback (× 4) when no resolver is passed.
 */
export function computeRhythm(history, { weeklyStrengthDays = RHYTHM_DEFAULT_WEEKLY, weekFor = null } = {}) {
  const now = Date.now();
  ...
  const days = new Set(
    (Array.isArray(history) ? history : [])
      ...
      .map(rec => rec.date || rec.id.slice(0, 10)),
  );
  const completed = days.size;
  let expected = Math.max(1, weeklyStrengthDays || RHYTHM_DEFAULT_WEEKLY) * 4;
  if (weekFor) {
    const today = todayLocalIso();
    let n = 0;
    for (let i = 0; i < RHYTHM_WINDOW_DAYS; i++) {
      const iso = addDaysIso(today, -i);
      if (weekFor(iso)?.[mondayIndex(iso)]?.type === "strength") n += 1;
    }
  ...
  return {
    completed,
    expected,
    ratio: Math.min(1, completed / expected),
...
export const H = {
  ...
  // Merge remote history into local — delegates to the shared module.
  merge: (local, remote) => mergeHistories(local, remote),
};
...
// Week validation + schedule-history normalisation moved to
// lib/sync-merge.js (shared with the server route). Imported above.

// Kept as a thin alias so existing call sites read cleanly; the one
// implementation lives in lib/dates.js.
...
export const W = {
  ...
  //
  // Resolution: the LATEST-EDITED entry whose effectiveFrom ≤ dateStr
  // (scheduleEntryOn, shared with the MCP server) — a save replaces the
  // plan from its effectiveFrom onward.
  getEffectiveOn: (dateStr) => {
    if (!_isIsoDate(dateStr)) return null;
    const best = _scheduleEntryOn(_ensureScheduleHistory(LS.get("forge:weekConfig", null)), dateStr);
  ...
  // Save a new effective week. Default effectiveFrom = today; the week
  // editor goes through saveEdit (this week's Monday).
  ...
  // The week editor's save: the edit applies from this week's Monday, so
  // this week's untrained days re-read under it (Thursday switched to
  // cardio on Friday stops being owed as strength). Earlier weeks keep
  // their plan; days already done still show what was done. Repeat saves
  // within a week collapse onto the one Monday entry.
  saveEdit: (week, todayIso = _todayLocalIso()) =>
    W.save(week, { effectiveFrom: mondayOfWeekIso(todayIso) || todayIso }),
...
export const PB = {
  ...
  // Mutations stamp updatedAt (the same-block-number merge tie-breaks on
  // it — rotation audit finding 8); the sync path passes touch=false so a
  // pulled block keeps its own stamp.
  save: (pb, { touch = true } = {}) => LS.set("forge:programmeBlock",
    touch ? { ...pb, updatedAt: new Date().toISOString() } : pb),
  ...
  // Route through save() so the reset stamps updatedAt. The raw LS.set it
  // used to do left the OLD stamp on the cleared state, so the same-block-
  // number merge tie-break preferred the pre-reset remote copy and the
  // cleared rotation RESURRECTED on the next pull (audit #4: deletion must
  // be expressed as a newer state, not an unstamped absence).
  reset: () => {
    const next = { ...PB.get(), config: {}, history: {}, updatedAt: new Date().toISOString() };
    PB.save(next, { touch: false }); // already stamped — keep stored === returned
    return next;
  },
};
...
//
// The Day entity rationale and invariants live in the internal design notes.
export const Days = {
  ...
  projectCurrentWeek: (profile, { now = new Date() } = {}) => {
    ...
    const monday = mondayOfWeekIso(now);
    const all = Days.getAll(profile);
    for (let i = 0; i < 7; i++) {
  ...
  // session — strength completion is history-backed). Read by /diag-sync
  // only; Home (picker, absence nudge) reads the day resolver.
...
// Day-of-week index (0=Mon..6=Sun) from an ISO date; null when malformed.
const _dowOfDate = (dateStr) => mondayIndex(dateStr);

// Add N days to an ISO date string. Callers here treat a malformed date as
// pass-through rather than null — the ?? preserves that.
const _addDays = (dateStr, n) => addDaysIso(dateStr, n) ?? dateStr;
...
export const F = {
  ...
  save: (profile, focus) => {
    ...
    LS.set(`forge:${profile}:focusStamp`, new Date().toISOString());
    return focus;
  },
  getStamp: (profile) => (profile ? LS.get(`forge:${profile}:focusStamp`, null) : null),
  saveRaw: (profile, focus, stamp) => {
    if (!profile) return;
    LS.set(`forge:${profile}:focus`, focus);
    if (stamp) LS.set(`forge:${profile}:focusStamp`, stamp);
  },
};

// ─── Travel mode (per profile, device-local) ─────────────────────────────────
// STICKY by design: a trip lasts days, so the check-in remembers it instead of
// asking every session. Off is the default and the only way back is the same
// toggle — nothing expires it behind the user's back.
//
// DEVICE-LOCAL on purpose, and the reason is design principle #0's other half:
// this describes where the device is right now, not who the user is. Syncing
// it would put the phone left at home into travel mode. It is registered in
// PROFILE_SUFFIXES so store-health knows it and a profile wipe clears it, but
// it never enters the sync payload.
export const TRAVEL = {
  get: (profile) => (profile ? LS.get(`forge:${profile}:travel`, false) === true : false),
  set: (profile, on) => {
    if (!profile) return false;
    LS.set(`forge:${profile}:travel`, !!on);
    return !!on;
  },
};

// ─── Absence nudge dismissal (per profile, device-local) ─────────────────────
// Absence itself stays derived (lib/absence.js); this only remembers which
// absence the user waved away, keyed by its start date, so the nudge stays
// gone across reloads until a NEW absence begins. One string, overwritten in
// place — never deleted, never swept. Device-local: it is a UI courtesy, and
// a nudge dismissed on the phone reappearing on the laptop costs one tap.
// Registered in PROFILE_SUFFIXES so store-health knows it and a wipe clears it.
export const AN = {
  key: (profile) => `forge:${profile}:absenceNudge`,
  get: (profile) => {
    if (!profile) return null;
    const v = LS.get(AN.key(profile), null);
    return typeof v === "string" ? v : null;
  },
  dismiss: (profile, startIso) => {
    if (!profile || typeof startIso !== "string") return false;
    return LS.set(AN.key(profile), startIso);
  },
};
...
//   hidden   → lifecycle flush (delta-era: dirty-diff decides; free when clean)
//   online   → flush push queue + pull
//
// THE TWO-TIER PUSH MODEL IS RETIRED (#3, PR C). The class-2 deferral tier
// existed to save blob ops on cheap mutations — with the delta protocol a
// push with nothing dirty never touches the wire, so the cost the tier
// deferred no longer exists. It had accumulated zero callers; deleted as
// obsolete rather than wired up. the old tiered-push design is historical.
...
// Push the current local snapshot now.
//
// Delta-era (#2 family, PR B): when a sync cursor exists (this device has
// hydrated from the DB era), diff against the last acknowledged push and
// ship ONLY dirty fields + new records. The diff decides WHICH fields of
// getLocalProfile's output travel — it never builds a payload of its own
// (the #1/S1 hand-rolled-subset class stays dead). No cursor → the fat
// push, exactly as before.
export async function pushNow(profile, { keepalive = false } = {}) {
  if (!profile) return false;
  const data = getLocalProfile(profile);
  if (isKnownPasskeyless(profile)) return blobPush(profile, data);
  if (!DeltaSync.getCursor(profile)) return blobPush(profile, data);

  const state = DeltaSync.getPushState(profile);
  // One-time reconciliation (2026-07-26). A device whose push-state predates
  // the pushKey watermark may be sitting on retro sessions the old code
  // silently never shipped — they are still in local history, and a full
  // push is the only thing that carries a record already BELOW the
  // watermark. blobPush commits fresh push-state (stamped with the new
  // version), so this fires exactly once per device and never again.
  if (needsReconcile(state)) return blobPush(profile, data);
  const { dirty } = DeltaSync.diffMeta(data.meta, state.fieldHashes);
  // A value map and its stamp map ship together (weights/weightStamps,
  // mainLifts/mainLiftStamps …). The server merges only what arrives, so a
  // stamps-only delta would be compared against nothing and dropped.
  for (const f of fieldClosure(Object.keys(dirty))) {
    if (!(f in dirty) && data.meta[f] !== undefined) dirty[f] = data.meta[f];
  }
  const records = DeltaSync.newRecords(data.history, state.lastRecordId);
  if (!Object.keys(dirty).length && !records.length) {
    PQ.clear(profile); // nothing outstanding — a queued retry is satisfied
    // A no-op sync IS a successful sync check (boss find, 2026-07-28):
    // "Sync now" with nothing dirty must still reset the last-sync clock,
    // or the UI counts up from a stale moment while claiming success.
    SyncStatus._set({ state: "idle", lastSync: Date.now(), error: null });
    return true;
  }
  const ok = await blobPushDelta(profile, { meta: dirty, history: records }, { keepalive });
  if (ok) DeltaSync.commitPushState(profile, data);
  return ok;
}

// Lifecycle flush — delta-era devices run the dirty-diff (free when clean,
// keepalive'd when something ships); pre-cursor devices no-op here exactly
// as the dead deferral tier always did for them (their save-point is the
// per-mutation pushNow they already ran).
export async function flushOnLifecycle(profile) {
  if (!profile || !DeltaSync.getCursor(profile)) return false;
  return pushNow(profile, { keepalive: true });
}

// #42: lifecycle flushes work on EVERY route. enableAutoSync only runs while
// ForgeApp (home) is mounted, so off-home routes (locker room, diag pages)
// used to have dead listeners — backgrounding there was a data-loss window.
// Off-home we run the flush-only half (durability: queued + deferred pushes);
// the pull-refresh half (backgroundSync) stays home-only because merging
// remote state into the UI needs the mounted React callback.
const _lifecycleProfile = () => _autoSyncProfile || P.getActive();

function _handleVisibilityChange() {
  if (document.visibilityState === "visible") {
    if (_autoSyncProfile) backgroundSync(_autoSyncProfile, { onUpdate: _autoSyncCallback });
    else flushPendingPushes(); // off-home: retry queued failures, no pull
  } else if (document.visibilityState === "hidden") {
    // keepalive (#12): a backgrounding tab may be killed mid-flight; the
    // browser completes keepalive-flagged requests. Only effective in the
    // delta era (small bodies) — blobPushDelta enforces the 64KB budget.
    flushOnLifecycle(_lifecycleProfile());
  }
}
...
function _handleOnline() {
  // Flush pending pushes with the ONE payload builder (getLocalProfile
  // default) — never a hand-rolled subset; see flushPendingPushes.
  flushPendingPushes();
  if (_autoSyncProfile) {
...
// Safari has supported navigator.storage.persist() since 17 (per the
// WebKit storage-policy update), and iOS evicts storage from
// little-used origins — for a local-first app whose localStorage is the
// recovery source when the blob transiently lacks fields, eviction
// protection is cheap survivability. Fire-and-forget: a false return is
// fine (the blob remains canonical), and Background Sync is NOT a
// fallback here — it remains unavailable on Apple platforms (checked
// against the 2026-07 WebKit research; recorded in the internal notes
...
// ── J1: "needs auth" is not an error ───────────────────────────────────────
// Sync is profile-bound now (2026-07-26). A 401 means this device has not
// proved it owns this profile — a passkey-less profile, or a sliding sync
// cookie that lapsed after a week away. That is a NORMAL, expected state for
// a local-first app, not a failure: nothing was lost, the app keeps working
// entirely from localStorage, and one ceremony resolves it. Surfacing it as a
// red "Sync failed" would be a lie that teaches users to ignore sync errors.
//
// Pushes still queue on 401 (PQ.add) so the moment the user signs in, the
// backlog goes up. Nothing is dropped for want of a cookie.
export const SYNC_STATE_NEEDS_AUTH = "needsAuth";
const authFail = (res) => res && res.status === 401;

export async function blobPull(profile) {
  SyncStatus._set({ state: "pulling", error: null });
  try {
    const res = await fetchWithTimeout(`/api/sync?profile=${encodeURIComponent(profile)}`);
    if (!res.ok) {
      if (authFail(res)) {
        SyncStatus._set({ state: SYNC_STATE_NEEDS_AUTH, error: null });
        return null;
      }
...
export async function blobPush(profile, data) {
  // No passkey, nothing the server will keep: queue locally, send nothing.
  if (isKnownPasskeyless(profile)) {
    PQ.add(profile);
    SyncStatus._set({ state: SYNC_STATE_NEEDS_AUTH, error: null });
    return false;
  }
  ...
  try {
    ...
    if (authFail(res)) {
      PQ.add(profile);   // keep the backlog — it ships once signed in
      SyncStatus._set({ state: SYNC_STATE_NEEDS_AUTH, error: null });
      return false;
    }
    if (!res.ok) throw new Error(`Push failed: ${res.status}`);
    PQ.clear(profile);
    // A full push brings the server current — acknowledge it in the delta
    // push-state so the next delta doesn't re-ship the world. Single choke
    // point: every full-push caller (backgroundSync push-back, pending
    // flush, legacy pushNow) lands here.
    DeltaSync.commitPushState(profile, data || {});
    SyncStatus._set({ state: "idle", lastSync: Date.now(), error: null });
    return true;
  } catch (e) {
    PQ.add(profile);
...
// ── Delta wire calls (#2 family, PR B) ────────────────
// Small payloads by construction. keepalive is only attached when requested
// AND the body fits the browser's ~64KB keepalive budget (#12): an oversize
// keepalive fetch REJECTS outright, which would turn the pagehide save-point
// into a guaranteed failure.
const KEEPALIVE_BUDGET_BYTES = 60_000;

export async function blobPushDelta(profile, delta, { keepalive = false } = {}) {
  SyncStatus._set({ state: "pushing", error: null });
  try {
    const body = JSON.stringify({ profile, delta });
    const res = await fetchWithTimeout("/api/sync", {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body,
      ...(keepalive && body.length < KEEPALIVE_BUDGET_BYTES ? { keepalive: true } : {}),
    });
    if (authFail(res)) {
      PQ.add(profile);
      SyncStatus._set({ state: SYNC_STATE_NEEDS_AUTH, error: null });
...
export async function blobPullDelta(profile, cursor) {
  SyncStatus._set({ state: "pulling", error: null });
  try {
    const res = await fetchWithTimeout(`/api/sync?profile=${encodeURIComponent(profile)}&since=${encodeURIComponent(cursor)}`);
    if (!res.ok) {
      // An invalid cursor is unrecoverable by retry — drop it so the next
      // cycle re-hydrates via the full pull. Transient failures keep it.
      if (res.status === 400) DeltaSync.clearCursor(profile);
      if (authFail(res)) {
        SyncStatus._set({ state: SYNC_STATE_NEEDS_AUTH, error: null });
        return null;
      }
...
// mergeScheduleHistory / mergeBreaks / mergeDayEntries / mergeMeta /
// mergeProfileData all live in lib/sync-merge.js now — one merge,
// imported by this client layer AND the server PUT route (audit S3).
...
export function getLocalProfile(profile) {
  return {
    meta: {
      ...
      weightStamps: P.getWeightStamps(profile),
      reps: P.getReps(profile),
      repStamps: P.getRepStamps(profile),
      ...
      userFocusUpdatedAt: F.getStamp(profile),
      // Main-lift choices + per-lift stamps. A choice made on one device
      // follows the user to the next, and the server can build the
      // programme they actually run (connected AI).
      mainLifts: P.getMainLifts(profile),
      mainLiftStamps: P.getMainLiftStamps(profile),
      ...
      // Bodyweight journal — the scale-moment trail the Locker Room chart
      // draws. Date-keyed, merged per-date by updatedAt (mergeBwLog).
      bodyweightLog: BW.getLogRaw(profile),
...
/** Writes merged remote state into the local cache.
 *
 *  Returns TRUE only if every write landed. A local write can fail under
 *  storage pressure, and the caller must not advance its sync cursor past
 *  rows that never made it — those rows are then behind the cursor and are
 *  never pulled again. Re-pulling is free: the merge is idempotent.
 */
function persistToLocal(profile, { meta, history }) {
  const failuresBefore = LS._quotaFailures || 0;
  // Raw setters, deliberately: the merged maps arrive with their own
  // per-key stamps; the diff-stamping savers would re-stamp remote-won
  // keys as freshly-local and inflate their authority (audit S2).
  P.saveWeightsRaw(profile, meta.weights || {}, meta.weightStamps || {});
  P.saveRepsRaw(profile, meta.reps || {}, meta.repStamps || {});
  // Only when the merged meta carries the field: a server that predates it
  // must not blank a device's choices.
  if (meta.mainLifts && typeof meta.mainLifts === "object") {
    P.saveMainLiftsRaw(profile, meta.mainLifts, meta.mainLiftStamps || {});
  }
  if (meta.streak) P.saveStreak(profile, meta.streak);
  if (meta.programmeBlock) PB.save(meta.programmeBlock, { touch: false });
  ...
  if (meta.userFocus) F.saveRaw(profile, meta.userFocus, meta.userFocusUpdatedAt || null);
  ...
  if (meta.bodyweightLog && typeof meta.bodyweightLog === "object" && Object.keys(meta.bodyweightLog).length > 0) {
    BW.saveLogRaw(profile, meta.bodyweightLog);
  }
  if (meta.trainingState && (
    Object.keys(meta.trainingState.lifts || {}).length > 0 ||
    Object.keys(meta.trainingState.muscleAnchors || {}).length > 0
  )) {
    TS.replaceState(profile, meta.trainingState, { touch: false });
  }
  ...
  return (LS._quotaFailures || 0) === failuresBefore;
}
...
export function backgroundSync(profile, { onUpdate, onError } = {}) {
  ...
  // ── Delta pull (#2 family, PR B) ──────────────────────────────────────
  // With a cursor, fetch only what changed. The partial arrives as the
  // REMOTE side of THE merge — the algebra treats absent fields as "keep
  // local" (each rule falls back when its remote side is missing), so a
  // quiet delta is a no-op and never clobbers. Two deliberate differences
  // from the full path: the cursor advances (pulls own it), and there is
  // NO localHadMore push-back — against a partial, local always looks
  // richer; outbound traffic is the dirty-diff push's job.
  const cursor = DeltaSync.getCursor(profile);
  if (cursor) {
    return blobPullDelta(profile, cursor).then(body => {
      if (!body || body.delta !== true) return { source: "local", changed: false };
      const merged = mergeProfileData(local, { meta: body.meta || {}, history: body.history || [] });
      // Cursor advances ONLY on a clean persist. If a local write failed,
      // the rows it carried would sit behind the new cursor and never be
      // pulled again — silent, permanent divergence on this device. Holding
      // the cursor costs one redundant pull; the merge is idempotent.
      if (persistToLocal(profile, merged)) {
        DeltaSync.setCursor(profile, body.cursor);
      } else {
        console.error("[forge:sync] local persist incomplete — holding cursor for re-pull");
      }
      if (merged.remoteHadMore && onUpdate) {
        onUpdate({ meta: merged.meta, history: merged.history, source: "delta" });
      }
      return { source: merged.remoteHadMore ? "delta" : "local", changed: merged.remoteHadMore };
    }).catch(err => {
  ...
  return blobPull(profile).then(remote => {
    ...
    const persisted = persistToLocal(profile, merged);
    if (!persisted) {
      console.error("[forge:sync] local persist incomplete — not entering delta mode this pull");
    }

    // Full pulls hand out a cursor in the DB era (PR A) — adopting it here
    // is how a device ENTERS delta mode. Pre-DB responses carry none.
    // Gated on a clean persist for the same reason as the delta path, plus
    // one more: acknowledging the push state below would mark fields clean
    // that never reached local storage.
    if (remote.cursor && persisted) {
      DeltaSync.setCursor(profile, remote.cursor);
      // Acknowledge the merged state as pushed (boss find, 2026-07-26: a
      // freshly-hydrated device showed ALL fields dirty — the phantom-14).
      // When local had nothing extra, server == merged, so acknowledging is
      // simply true. When local HAD more, the push-back below runs through
      // blobPush, whose choke point commits — so this branch only covers
      // the quiet case. Committing `merged` (not the re-read local) keeps
      // any representational drift honest: it shows dirty once, ships, and
      // self-heals rather than being silently marked clean.
      if (!merged.localHadMore) DeltaSync.commitPushState(profile, merged);
    }
...
export async function checkProfileExists(profile) {
  try {
    const res = await fetchWithTimeout(`/api/sync?profile=${encodeURIComponent(profile)}&check=1`);
...
export async function blobDelete(profile, { authToken } = {}) {
  try {
    // The wipe token travels in a HEADER, never the query string (deep
    // audit 2026-07-26). It is the single credential authorising
    // irreversible deletion, and a URL lands in access logs, proxy logs and
    // Referer. /api/photos has always obeyed this law; the wipe now does too.
    const url = `/api/sync?profile=${encodeURIComponent(profile)}`;
    const res = await fetchWithTimeout(url, {
      method: "DELETE",
      ...(authToken ? { headers: { "X-HW-Auth": authToken } } : {}),
    });
    if (!res.ok) {
...
// dataFn defaults to getLocalProfile — THE payload builder. A retried push
// overwrites blob meta wholesale (the server merges history, not meta), so
// a hand-rolled partial payload here DELETES every omitted field from the
// canonical blob. That exact bug shipped: the ForgeApp mount retry carried
// only {weights, reps, streak, programmeBlock} and wiped days / userWeek /
// userFocus / bodyweight / trainingState / breaks on every app-open retry
// (sync audit S1, 2026-07-13). Callers should pass NOTHING; the parameter
// survives only for tests.
export async function flushPendingPushes(dataFn = getLocalProfile) {
  const pending = PQ.get();
...
export function weekKey() {
  // Local Monday of the current week (was UTC — split bonus marks across two
  // keys near local midnight for non-UTC users). See lib/dates.js.
  return mondayOfWeekIso(new Date());
}
...
export function dateOfWeekdayIdxInCurrentWeek(weekdayIdx) {
  // Composed from lib/dates.js rather than re-deriving the Monday shift and
  // the y/m/d formatting by hand — this file already imports both helpers a
  // few lines up for weekKey(). Hand-rolled copies are how the local-vs-UTC
  // date class keeps coming back (third occurrence, 2026-07-26); the guard in
  // tests/date-doctrine.test.js now enforces the single source.
  return addDaysIso(mondayOfWeekIso(new Date()), weekdayIdx);
}
...
export function rpeToRir(rpe) {
  // Numeric RPE (the continuous 6–10 track, 0.5 steps): RIR = 10 − RPE,
  // half-steps preserved, clamped to the engine's 0–3 band. Records carry
  // number-or-string by era; enum strings below never migrate.
  if (typeof rpe === "number" && Number.isFinite(rpe)) {
    return Math.min(3, Math.max(0, Math.round((10 - rpe) * 2) / 2));
  }
...
export function inferLoadType(name) {
  ...
  // "leg curl" is deliberately NOT a blanket match (audit #58): machine/
  // seated/single-leg curls move external plates — only the ball/slider
  // family is genuinely bodyweight (nordic + copenhagen match separately).
  if (/(plank|hold|dead\s*bug|bird\s*dog|hollow|push-?up|crunch|(leg|knee)\s*raise|y-?t-?w|captain'?s\s*chair|row\s*\(trx|trx|nordic|copenhagen|glute\s*bridge|side\s*plank|(swiss\s*ball|stability\s*ball|slider)\s*leg\s*curl)/.test(n)) {
...
export const BW = {
  ...
  set: (profile, kg) => {
    ...
    const rounded = Math.round(kg * 10) / 10; // single decimal
    const updatedAt = new Date().toISOString();
    LS.set(BW.key(profile), { kg: rounded, updatedAt });
    // Journal the scale moment (boss, 2026-07-24: "there's no other way for
    // this feature to work"). Date-keyed, one entry per LOCAL calendar day,
    // last write of the day wins. Without this, standalone weight logs left
    // no trail — the Locker Room chart could only see session-start
    // snapshots and photo tags.
    const log = BW.getLogRaw(profile);
    log[todayLocalIso()] = { kg: rounded, updatedAt };
    LS.set(BW.logKey(profile), log);
  },

  // ── Bodyweight journal — date-keyed { "YYYY-MM-DD": { kg, updatedAt } } ──
  logKey: (profile) => `forge:${profile}:bodyweightLog`,

  getLogRaw: (profile) => {
    if (!profile) return {};
    const v = LS.get(BW.logKey(profile), null);
    return v && typeof v === "object" && !Array.isArray(v) ? v : {};
  },

  // Hydration path: land the MERGED journal as-is (mergeBwLog already
  // resolved per-date conflicts, so this is authoritative — same contract
  // as Days.replaceAll).
  saveLogRaw: (profile, log) => {
    if (!profile || !log || typeof log !== "object" || Array.isArray(log)) return;
    LS.set(BW.logKey(profile), log);
  },
...
export function newDraftLog({
  ...
  travel        = false,               // bodyweight session, away from the gym
}) {
  // The session's calendar day on the USER's clock. Was UTC
  // (toISOString().slice(0,10)) while `dow` below is local, so the two
  // disagreed and every non-UTC user's records were misdated by up to a day
  // (an Auckland morning session landed on "yesterday"; a US-West evening on
  // "tomorrow"/next week). Records are immutable and synced, so the misdating
  // was permanent. `id` stays a UTC instant — it's an ordering key, not a
  // calendar day. See lib/dates.js.
  const date = todayLocalIso();
  const { zone: loggedTz, offset: loggedTzOffset } = captureZone();
  return {
    ...
    // WHERE the clock was when this was written. `date` above is a bare
    // calendar day with no zone attached, which is unambiguous only until a
    // user crosses timezones — westward repeats a date, eastward skips one.
    // Capturing the zone is not itself the fix; it is what makes a fix
    // POSSIBLE later, because a record written without it can never be
    // disambiguated after the fact.
    //
    // Named for capture, not training: on a live session these coincide, but
    // a retro log stamps where the user was when they TYPED it, which may not
    // be where they trained. Same distinction as loggedAt vs id — do not read
    // this as the training location.
    loggedTz,
    loggedTzOffset,
    ...
    // Travel session (lib/travel.js): the A/B/C shape trained with bodyweight
    // away from the gym. THE PROGRESSION ENGINE MUST SKIP THESE — a hotel-room
    // push-up says nothing about tomorrow's bench prescription, and letting it
    // set one would quietly wreck a returning user's loads. The volume is real
    // and counts everywhere else. Absent on every pre-travel record, so read
    // it as `=== true`, never as "not false".
    travel: travel === true || undefined,
...
export function logSet(draft, {
  ...
  reach = false,                        // set taken BEYOND the prescription on a Fresh day
}) {
  ...
  // Keep RPE consistent. A numeric RPE is the primary capture — it is
  // stored exactly as dragged and never reconstructed from RIR. Otherwise
  // (enum-era callers): a precise caller-supplied RIR derives the RPE label,
  // keeping the two scales in sync on disk.
  const effectiveRpe = (typeof rpe === "number" && Number.isFinite(rpe))
    ? rpe
  ...
  bl.exercises[exerciseName].sets.push({
    ...
    est1rm,
    volume,
    // A reach — an extra set, or the last one flipped heavier, on a Fresh day.
    // Absent on every ordinary set (and on every pre-reach record), so the
    // engine's guard reads it the same way `travel` is read. lib/progression.js
    // holds the contract: never a miss, earned only, still honest.
    reach: reach === true || undefined,
  });
  return draft;
}
...
export function finaliseDraft(draft) {
  ...
  const blocks = Object.values(draft.blocks).map(b => {
    const exercises = Object.values(b.exercises).map(ex => {
      ...
      const nonEmptySets = (ex.sets || []).filter(isCountedSet);
      ...
      return {
        ...
        summary: {
          totalVolume,
          avgRir,
          topSet: topSet ? { weight: topSet.weight, reps: topSet.reps, rir: topSet.rir, rpe: topSet.rpe ?? null, est1rm: topSet.est1rm } : null,
          hitTarget,
        },
      };
    });
...
export function migrateV1ToV2(rec) {
  ...
  const blocks = (rec.blocks || []).map(b => {
    const exercises = (b.exercises || []).map(ex => {
      ...
      const nonEmpty = sets.filter(isCountedSet);
...
// The one implementation lives in lib/dates.js (already local-correct here;
// aliased so the denormalisation call sites read in context).
function _mondayOfDate(dateStr) {
  return mondayOfWeekIso(dateStr);
}
...
export const TS = {
  ...
  // touch=true bumps updatedAt — every MUTATION path does; the sync path
  // (persistToLocal) passes touch=false so a pulled remote state keeps its
  // own stamp instead of masquerading as freshly-local (audit S2: the
  // whole-object "remote wins" rule now compares these stamps).
  save: (profile, state, { touch = false } = {}) => {
    if (!profile) return;
    LS.set(TS.key(profile), {
      ...state,
  ...
  updateLift: (profile, liftName, liftState) => {
    ...
    TS.save(profile, next, { touch: true });
    return next;
  },
  ...
  replaceState: (profile, newState, { touch = true } = {}) => {
    if (!profile) return;
    TS.save(profile, newState, { touch });
    return newState;
  },
