...
export const Days = {
  ...
  // Existing Day entries are preserved; the projection only fills in dates
  // that aren't already present.
  _maybeProject: (profile) => {
    if (!profile) return;
    if (LS.get(Days._projectedKey(profile), false)) return;
  ...
  if (_autoSyncProfile) {
    ...
    flushPendingPushes((profile) => ({
      meta: {
        ...
        // Days entity — date-keyed unified completion record. Ships
        // alongside dayDone / bonusDone during the dual-write rollout so
        // a cross-device merge restores both surfaces. Once reads cut
        // over to Days, the legacy fields become safety-net duplicates.
...
export function getLocalProfile(profile) {
  return {
    meta: {
      ...
      dayDone: P.getDayDone(profile),
      bonusDone: P.getBonusDone(profile),
      // Days entity — unified date-keyed completion store. Ships in the
      // meta payload alongside the legacy dayDone / bonusDone during the
      // dual-write rollout.
      days: Days.getAll(profile),
