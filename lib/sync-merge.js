...
export function mergeProfileData(local, remote) {
  ...
  const mergedKey = stableStringify({ m: mergedMeta, h: mergedHistory });
  const remoteHadMore = mergedKey !==
    stableStringify({ m: mergeMeta(localMeta, localMeta), h: mergeHistories(localHistory, localHistory) });
  const localHadMore = mergedKey !==
    stableStringify({ m: mergeMeta(remoteMeta, remoteMeta), h: mergeHistories(remoteHistory, remoteHistory) });
